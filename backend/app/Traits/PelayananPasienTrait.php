...
trait PelayananPasienTrait
{
    ...
    protected function getProdukIdSewaAlat(){
        $set = SettingDataFixed::where('namafield', 'idProdukSewaAlat')->first();
        $this->id= ($set) ? (int)$set->nilaifield: null;
        return $this->id;
    }
    ...
    protected function getDepositSewaAlat($noregistrasi){
        $produkIdSewaAlat = $this->getProdukIdSewaAlat();
        $sewaalat = 0;
        $pasienDaftar  = PasienDaftar::has('pelayanan_pasien')->where('noregistrasi', $noregistrasi)->first();
        if($pasienDaftar){
            $sewaalatList =$pasienDaftar->pelayanan_pasien()->where('nilainormal', '-1')->whereNull('strukfk')->get();
            foreach ($sewaalatList as $item){
                if($item->produkfk==$produkIdSewaAlat){
                    $sewaalat = $sewaalat + $item->hargasatuan;
                }
            }
        }
        return $sewaalat;
...
//        }
    ...
    protected function getUserKeyAntreanBPJSv2(){
        ...
        if($statusBridgingProduction->nilaifield == 'false')
            ...
            $set = SettingDataFixed::where('namafield', 'UserKeyAntreanBPJS_prod')->first();
...
//    }
    protected function getUrlSisrute(){
        $set = SettingDataFixed::where('namafield', 'urlBridgingSisrute')
            ->where('kdprofile',$this->getKdProfile())->first();
        return $set->nilaifield;
    }
    protected function getUrlYankes(){
        $set = SettingDataFixed::where('namafield', 'urlBridgingYankes')
            ->where('kdprofile',$this->getKdProfile())->first();
        return $set->nilaifield;
    }
    protected function getIdConsumerBPJS(){
       ...
       $set = SettingDataFixed::where('namafield', 'idConsumerBPJS')
                    ->where('kdprofile',$this->getKdProfile())->first()->nilaifield;
    ...
    protected function getLinkAplicare(){
        $statusBridgingProduction = SettingDataFixed::where('namafield', 'isBridgingProduction')->first();
        $isTemporaryBrigding = SettingDataFixed::where('namafield', 'isTemporaryBrigding')->first();
        if(!empty($statusBridgingProduction) && $statusBridgingProduction->nilaifield == 'true'

        &&$isTemporaryBrigding->nilaifield == 'false' ){
                $set = SettingDataFixed::where('namafield', 'urlBridgingAplicare')
                    ->where('kdprofile',$this->getKdProfile())->first()->nilaifield;
        }else{
                $set = SettingDataFixed::where('namafield', 'urlBridgingAplicareDev')
                    ->where('kdprofile',$this->getKdProfile())->first()->nilaifield;
        }
        return $set;
    }
    ...
    protected function getUrlBrigdingNAR(){
        $statusBridgingProduction = SettingDataFixed::where('namafield', 'isNARBridgingProduction')->first();
        if(!empty($statusBridgingProduction)){
            if($statusBridgingProduction->nilaifield == 'false') {
                $set = SettingDataFixed::where('namafield', 'linkNARdev')->first();
            } else{
                $set = SettingDataFixed::where('namafield', 'linkNAR')->first();
            }
        }else{
            $set = SettingDataFixed::where('namafield', 'linkNAR')->first();
        }
