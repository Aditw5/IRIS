    ...
    <TabView class="tabview-custom " :scrollable="true">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Micrometer Head</span>
        ...
        <VCard>
          ...
          <div class="column is-12" v-else>
            ...
            <DataTable :value="dataSourcefilter" :paginator="true" :rows="10"
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Micrometer Head</h3>
              </div>
            </div>
...
useHead({
  title: 'Lembar Kerja Micrometer Head - ' + import.meta.env.VITE_PROJECT,
})
...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-micrometer-head?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`pelaksana/get-lembar-kerja-micrometer-head?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const onSelectedFiles = async (event: any) => {
  ...
  reader.onload = (e: any) => {
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      /**
       * Jika suatu saat template punya judul section/group:
       * contoh:
       * "Micrometer Head"
       * lalu di bawahnya header Rentang / Range
       *
       * Judul ini tetap dilewati dan TIDAK DISIMPAN.
       */
      if (
        nonEmptyCount === 1 &&
        typeof row[0] === 'string' &&
      ...
      /**
       * Template Excel Anda sekarang langsung mulai dari:
       * Baris 1: Rentang | Penunjukan Standar | Pembacaan Alat | dst
       * Baris 2: Range   | Calibrator Output | Instrument Reading | dst
       *
       * Jadi semua baris header dilewati, lalu setelah itu data mulai dibaca.
       */
       ...
       * Group benar-benar tidak dibuat dan tidak dikirim.
       */
...
const Save = async () => {
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/pelaksana/save-data-upload-lembar-kerja-micrometer-head', formData)
    isLoadingSave.value = false
