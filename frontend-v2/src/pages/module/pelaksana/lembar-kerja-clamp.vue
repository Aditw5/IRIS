...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-clamp?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`pelaksana/get-lembar-kerja-clamp?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/pelaksana/save-data-upload-lembar-kerja-clamp`, json).then((response: any) => {
      isLoadingSave.value = false
