        ...
        <VCard>
          <div class="column is-12">
            ...
            <DataTable v-model:editingRows="editingRowsSumber" :value="dataSourceHasilSumber" editMode="row"
              ...
              currentPageReportTemplate="Showing {first} to {last} of {totalRecords}" :loading="isLoading">
              <template #header>
                <div class="column pt-0 pb-0" v-if="dataSourceHasilSumber.length > 0">
                  <VButtons style="justify-content: space-between;">
                    <VButton color="primary" @click="cetakSertifikatLembarKerja()" outlined icon="feather:printer">
                      Cetak Sertifikat
                    </VButton>
...
useHead({
  title: 'Lembar Kerja Meter & Sumber - ' + import.meta.env.VITE_PROJECT,
})
...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-meter-sumber?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`pelaksana/get-lembar-kerja-meter-sumber?norecdetail=${NOREC_DETAIL}`);
    response.forEach((element: any, i: any) => {
      element.no = i + 1
    });
    dataSourceHasilMeter.value = response.filter((item: any) => item.jenis === 'meter')
    dataSourceHasilSumber.value = response.filter((item: any) => item.jenis === 'sumber')
  } catch (err) {
  ...
  if (files && files.length > 0) {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        const sumberGroups: typeof groups = [];

        let currentGroup: typeof meterGroups[number] | null = null;
        let nomor = 1;
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          if (row.filter(x => !!x).length === 1 && typeof row[0] === 'string' && row[0].length > 2) {
            ...
            isMeter = /calibrator output/i.test((jsonData[i + 2] || []).join(' '));
            if (isMeter) {
              meterGroups.push(currentGroup);
            } else {
              sumberGroups.push(currentGroup);
            }
            nomor = 1;
            headerRowCount = 0;
            continue;
        ...
        const flatSumber = sumberGroups.flatMap(group => [
          { isGroupHeader: true, group: group.group },
          ...group.data.map(row => ({ ...row, group: group.group, isGroupHeader: false }))
        ]);

        dataSourceMeter.value = flatMeter;
        dataSourceSumber.value = flatSumber;
      };
...
const dataSourcefilter = computed(() => {
  return [...dataSourceSumber.value, ...dataSourceMeter.value];
});
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/pelaksana/save-data-upload-lembar-kerja-meter-sumber`, json
  ).then((response: any) => {
    isLoadingSave.value = false;
...
const cetakSertifikatLembarKerja = () => {
  console.log(dataSourceHasilMeter.value)
