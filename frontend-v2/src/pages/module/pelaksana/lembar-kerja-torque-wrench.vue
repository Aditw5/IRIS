    ...
    <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Torque Wrench</span>
              ...
              <div class="column is-3 mt-4">
                <VCardCustom :style="'padding:5px 25px'">
                  <div class="label-status success">
                    ...
                    <span class="ml-1">TOTAL DATA TORQUE WRENCH</span>
                  </div>
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Torque Wrench </h3>
              </div>
            </div>
...
useHead({
  title: 'Lembar Kerja Torque Wrench - ' + import.meta.env.VITE_PROJECT,
})
...
const addNewAlatStandar = (items: any) => {
...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-torque-wrench?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`pelaksana/get-lembar-kerja-torque-wrench?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const dataSourcefilter = computed(() => {
  let tarifTotal = 0;
  const filteredData = dataSource.value.filter((data: any) => {
    return (
      (!filterd.value)
    );
  });

  tarifTotal = filteredData.reduce((total: number, data: any) => total + parseFloat(data.TOTAL_TARIF), 0);
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/pelaksana/save-data-upload-lembar-kerja-torque-wrench`, json).then((response: any) => {
      isLoadingSave.value = false
