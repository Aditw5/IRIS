        ...
        <VCard>
          <div class="column is-12">
            ...
            <DataTable v-model:editingRows="editingRows" :value="dataSourceHasilClamp" editMode="row" dataKey="no"
              ...
              <template #header>
                <div class="column pt-0 pb-0" v-if="dataSourceHasilClamp.length > 0">
...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-clamp-meter?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`pelaksana/get-lembar-kerja-clamp-meter?norecdetail=${NOREC_DETAIL}`);
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/pelaksana/save-data-upload-lembar-kerja-clamp-meter`, json
  ).then((response: any) => {
    isLoadingSave.value = false;
...
const cetakSertifikatLembarKerja = () => {
  console.log(dataSourceHasilClamp.value)

  H.printBlade(`pelaksana/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilClamp.value[0].norecregis}&norec_detail=${dataSourceHasilClamp.value[0].detailregistraifk}`);
}
