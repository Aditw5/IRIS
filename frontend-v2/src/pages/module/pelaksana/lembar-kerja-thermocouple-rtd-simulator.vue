        ...
        <VCard>
          <div class="column is-12">
            ...
            <DataTable v-model:editingRows="editingRows" :value="dataSourceHasilSetpoint" editMode="row" dataKey="norec"
              ...
              <template #header>
                <div class="column pt-0 pb-0" v-if="dataSourceHasilSetpoint.length > 0">
            ...
            <DataTable v-model:editingRows="editingRowsRentang" :value="dataSourceHasilRentang" editMode="row"
              ...
              currentPageReportTemplate="Showing {first} to {last} of {totalRecords}" :loading="isLoading">
              <template #header>
                <div class="column pt-0 pb-0" v-if="dataSourceHasilRentang.length > 0">
                  <VButtons style="justify-content: space-between;">
                    <VButton color="primary" @click="cetakSertifikatLembarKerja()" outlined icon="feather:printer">
                      Cetak Sertifikat
                    </VButton>
...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-thermocouple-rtd-simulator?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`pelaksana/get-lembar-kerja-thermocouple-rtd-simulator?norecdetail=${NOREC_DETAIL}`);
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/pelaksana/save-data-upload-lembar-kerja-thermocouple-rtd-simulator`, json
  ).then((response: any) => {
    isLoadingSave.value = false;
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`pelaksana/cetak-sertifikat-lembar-kerja?pdf=true&norec=${firstResult().norecregis}&norec_detail=${firstResult().detailregistraifk}`);
}
