    ...
    <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Meter</span>
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Meter </h3>
              </div>
            </div>
...
useHead({
  title: 'Lembar Kerja Meter - ' + import.meta.env.VITE_PROJECT,
})
...
const onSelectMeter = async (filez: any) => {
...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-meter?token=' + useUserSession().token, '_blank');
}
...
const Save = async () => {
  ...
  // Antisipasi kalau masih ada baris header/grup, dan pastikan kolom B–L kosong saat tidak ada nilai
  const rows = (dataSourcefilter.value || [])
    .filter((r: any) => !r?.isGroupHeader && toStr(r?.rentang) !== '') // hanya baris data dg kolom A terisi
    .map((r: any) => ({
      no: r?.no ?? '',
      group: toStr(r?.group),
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/pelaksana/save-data-upload-lembar-kerja', formData)
    isLoadingSave.value = false
