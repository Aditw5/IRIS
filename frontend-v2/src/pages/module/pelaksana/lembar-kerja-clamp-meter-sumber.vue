      ...
      <TabPanel>
        ...
        <VCard>
          ...
          <div class="column is-12" v-else>
              ...
              <div class="column is-3 mt-4">
                <VCardCustom :style="'padding:5px 25px'">
                  <div class="label-status success">
                    ...
                    <span class="ml-1">TOTAL DATA METER & SUMBER</span>
                  </div>
            ...
            <!-- ==================== CLAMP ==================== -->
            <h4>Clamp</h4>
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourceClamp" :paginator="true" :rows="10"
            ...
            <!-- ==================== METER ==================== -->
            <h4>Meter</h4>
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourceMeter" :paginator="true" :rows="10"
              :rowsPerPageOptions="[5, 10, 25]" :loading="isLoading" showGridlines responsiveLayout="stack"
              breakpoint="960px" currentPageReportTemplate="Showing {first} to {last} of {totalRecords}">
              <ColumnGroup type="header">
                <Row>
            ...
            <!-- ==================== SUMBER ==================== -->
            <h4>Sumber</h4>
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourceSumber" :paginator="true" :rows="10"
              :rowsPerPageOptions="[5, 10, 25]" :loading="isLoading" showGridlines responsiveLayout="stack"
              breakpoint="960px" currentPageReportTemplate="Showing {first} to {last} of {totalRecords}">
              <ColumnGroup type="header">
                <Row>
        ...
        <VCard>
          <div class="column is-12">
            ...
            <DataTable v-model:editingRows="editingRowsClamp" :value="dataSourceHasilClamp" editMode="row" dataKey="no"
              @row-edit-save="onRowEditSaveClamp" rowGroupMode="rowspan" groupRowsBy="group" :paginator="true"
              ...
              <template #header>
                <div class="column pt-0 pb-0" v-if="dataSourceHasilMeter.length > 0">
            ...
            <DataTable v-model:editingRows="editingRowsSumber" :value="dataSourceHasilSumber" editMode="row"
              dataKey="no" @row-edit-save="onRowEditSaveSumber" rowGroupMode="rowspan" groupRowsBy="group"
...
const dataSourceMeter: any = ref([])
const dataSourceSumber: any = ref([])
const dataSourceClamp: any = ref([])
...
// RAW (untuk dikirim ke backend termasuk header group)
const rawMeter: any = ref([])
const rawSumber: any = ref([])
const rawClamp: any = ref([])
...
const onRowEditSave = async (event: any) => {
  ...
  dataSourceHasilMeter.value[index] = newData
...
const onRowEditSaveSumber = async (event: any) => {
  const { newData, index } = event
  dataSourceHasilSumber.value[index] = newData
  const payload = { data: [newData], sublingkupfk: SUBLINGKUP, norec_detail: NOREC_DETAIL }
  isLoadingSave.value = true
  try { await useApi().post(`/pelaksana/edit-lembar-kerja`, payload) }
  catch (err: any) { console.error(err) }
...
const onRowEditSaveClamp = async (event: any) => {
  const { newData, index } = event
  dataSourceHasilClamp.value[index] = newData
  const payload = { data: [newData], sublingkupfk: SUBLINGKUP, norec_detail: NOREC_DETAIL }
  isLoadingSave.value = true
  try { await useApi().post(`/pelaksana/edit-lembar-kerja`, payload) }
  catch (err: any) { console.error(err) }
...
const fetchAlatStandar = async (filter: any) => {
  await useApi().get(`pelaksana/alat-standar?param_search=namaalatstandar,namamerk,namatipe,namaserialnumber&query=${filter.query}`)
    .then((response) => {
...
const fetchRuangan = async (filter: any) => {
  await useApi().get(`general/dropdown/ruangan_m?select=id,namaruangan&param_search=namaruangan&query=${filter.query}&limit=10`)
    .then((response) => { d_ruangan.value = response })
}

const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-clamp-meter-sumber?token=' + useUserSession().token, '_blank')
}
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`pelaksana/get-lembar-kerja-clamp-meter-sumber?norecdetail=${NOREC_DETAIL}`)
...
/** ==== HELPERS ==== */
const makeFlat = (groups: any[], jenis: 'meter' | 'sumber' | 'clamp') => {
  return groups.flatMap(group => ([
    { isGroupHeader: true, group: group.group, jenis },
    ...group.data.map((row: any) => ({ isGroupHeader: false, group: group.group, jenis, ...row }))
  ]))
}
/** ================= */
  ...
  if (files && files.length > 0) {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        const meterGroups: any[] = []
        const sumberGroups: any[] = []
        const clampGroups: any[] = []
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          // deteksi baris judul group (1 sel berisi teks relatif panjang)
          if (row.filter(x => !!x).length === 1 && typeof row[0] === 'string' && row[0].length > 2) {
            currentGroup = { group: row[0], data: [] }

            const titleLower = row[0].toLowerCase()
            const next2 = (jsonData[i + 2] || []).join(' ').toLowerCase()

            isClamp = /clamp/.test(titleLower) || /current coil/.test(titleLower) || /clamp meter/.test(titleLower)
            isMeter = !isClamp && (/penunjukan standar/.test(next2) || /calibrator output/.test(next2))

            if (isClamp) clampGroups.push(currentGroup)
            else if (isMeter) meterGroups.push(currentGroup)
            else sumberGroups.push(currentGroup)
          ...
          if (currentGroup && headerRowCount === 2 && row[0] && row[2]) {
            ...
            if (isMeter) {
              ...
              currentGroup.data.push({
                ...commonFields,
                penyetelan_sumber: row[2] ?? '',
                penyetelan_sumber_satuan: row[3] ?? '',
                penyetelan_sumber_2: row[4] ?? '',
                penyetelan_sumber_satuan_2: row[5] ?? '',
                pembacaan_alat: row[6] ?? '',
                pembacaan_alat_satuan: row[7] ?? ''
              })
            } else {
        ...
        // FLATTEN: sertakan header group (isGroupHeader:true) dan baris data (isGroupHeader:false)
        const flatMeter = makeFlat(meterGroups, 'meter')
        const flatSumber = makeFlat(sumberGroups, 'sumber')
        const flatClamp = makeFlat(clampGroups, 'clamp')

        rawMeter.value = flatMeter
        rawSumber.value = flatSumber
        rawClamp.value = flatClamp

        // tampilan tabel: tanpa header
        dataSourceMeter.value = flatMeter.filter((r: any) => !r.isGroupHeader)
        dataSourceSumber.value = flatSumber.filter((r: any) => !r.isGroupHeader)
        dataSourceClamp.value = flatClamp.filter((r: any) => !r.isGroupHeader)
      }
...
const onRemoveTemplatingFile = (file: any, removeFileCallback: any, index: any) => {
  ...
  dataSourceSumber.value = []
  dataSourceMeter.value = []
  dataSourceClamp.value = []
...
// Kirim RAW (termasuk isGroupHeader) ke backend
const dataSourcefilter = computed(() => {
  return [...rawSumber.value, ...rawMeter.value, ...rawClamp.value]
})
...
const Save = async () => {
  ...
  isLoadingSave.value = true
  await useApi().post(`/pelaksana/save-data-upload-lembar-kerja-clamp-meter-sumber`, json)
    .then(() => {
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`pelaksana/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilMeter.value[0].norecregis}&norec_detail=${dataSourceHasilMeter.value[0].detailregistraifk}`)
}
