...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-feeler-gauge?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`pelaksana/get-lembar-kerja-feeler-gauge?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const Save = async () => {
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/pelaksana/save-data-upload-lembar-kerja-feeler-gauge', formData)
    isLoadingSave.value = false
