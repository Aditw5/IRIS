      ...
      <TabPanel>
        ...
        <VCard>
          ...
          <div class="column is-12" v-else>
              ...
              <div class="column is-3 mt-4">
                <VCardCustom :style="'padding:5px 25px'">
                  <div class="label-status success">
                    ...
                    <span class="ml-1">TOTAL DATA CONTINUITY +SOURCE </span>
                  </div>
            ...
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourceContinuity" :paginator="true"
        ...
        <VCard>
          <div class="column is-12">
            ...
            <h4>Resistance Continuity</h4>
            <DataTable v-model:editingRows="editingRows" :value="dataSourceHasilContinuity" editMode="row" dataKey="no"
              ...
              <template #header>
                <div class="column pt-0 pb-0" v-if="dataSourceHasilContinuity.length > 0">
            ...
            <h4>Source</h4>
            <DataTable v-model:editingRows="editingRowsSource" :value="dataSourceHasilSource" editMode="row" dataKey="no"
              @row-edit-save="onRowEditSaveSource" rowGroupMode="rowspan" groupRowsBy="group" :paginator="true"
...
const dataSourceContinuity: any = ref([])
...
const onRowEditSave = async (event: any) => {
  ...
  dataSourceHasilContinuity.value[index] = newData
  const payload = {
    data: [newData],
...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-continuity-source?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`pelaksana/get-lembar-kerja-continuity-source?norecdetail=${NOREC_DETAIL}`);
      ...
      reader.onload = (e) => {
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          if (row.filter(x => !!x).length === 1 && typeof row[0] === 'string' && row[0].length > 2) {
            ...
            isContinuity = /calibrator output|effective current output/i.test((jsonData[i + 2] || []).join(' '));
            if (isContinuity) {
        ...
        const flatContinuity = continuityGroups.flatMap(group => [
          { isGroupHeader: true, group: group.group },
          ...group.data.map(row => ({ ...row, group: group.group, isGroupHeader: false }))
        ]);

        const flatSource = sourceGroups.flatMap(group => [
          { isGroupHeader: true, group: group.group },
          ...group.data.map(row => ({ ...row, group: group.group, isGroupHeader: false }))
        ]);

        dataSourceContinuity.value = flatContinuity;
...
const onRemoveTemplatingFile = (file: any, removeFileCallback: any, index: any) => {
  ...
  dataSourceContinuity.value = [];
  dataSourceSource.value = [];
  valueProgress.value = 0;
};

const dataSourcefilter = computed(() => {
  return [...dataSourceContinuity.value, ...dataSourceSource.value];
});
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/pelaksana/save-data-upload-lembar-kerja-continuity-source`, json
  ).then((response: any) => {
    isLoadingSave.value = false;
...
const cetakSertifikatLembarKerja = () => {
  console.log(dataSourceHasilContinuity.value)

  H.printBlade(`pelaksana/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilContinuity.value[0].norecregis}&norec_detail=${dataSourceHasilContinuity.value[0].detailregistraifk}`);
}
