...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-bore-gauge?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`pelaksana/get-lembar-kerja-bore-gauge?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const Save = async () => {
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/pelaksana/save-data-upload-lembar-kerja-bore-gauge', formData)
    isLoadingSave.value = false
...
const detailOrder = async () => {
  ...
  item.value.sndial = data.sndiallembarkerja;
