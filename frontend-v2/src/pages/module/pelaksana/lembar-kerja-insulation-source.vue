        ...
        <VCard>
          <div class="column is-12">
            ...
            <DataTable v-model:editingRows="editingRows" :value="dataSourceHasilInsulation" editMode="row" dataKey="no"
              ...
              <template #header>
                <div class="column pt-0 pb-0" v-if="dataSourceHasilInsulation.length > 0">
...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-insulation-source?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`pelaksana/get-lembar-kerja-insulation-source?norecdetail=${NOREC_DETAIL}`);
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/pelaksana/save-data-upload-lembar-kerja-insulation-source`, json
  ).then((response: any) => {
    isLoadingSave.value = false;
...
const cetakSertifikatLembarKerja = () => {
  console.log(dataSourceHasilInsulation.value)

  H.printBlade(`pelaksana/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilInsulation.value[0].norecregis}&norec_detail=${dataSourceHasilInsulation.value[0].detailregistraifk}`);
}
