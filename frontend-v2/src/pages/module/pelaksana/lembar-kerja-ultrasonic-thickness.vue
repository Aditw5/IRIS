...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-ultrasonic-thickness?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`pelaksana/get-lembar-kerja-ultrasonic-thickness?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const Save = async () => {
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/pelaksana/save-data-upload-lembar-kerja-ultrasonic-thickness', formData)
    isLoadingSave.value = false
