      ...
      <TabPanel>
        ...
        <VCard>
          <div class="column is-12">
            ...
            <h4>Clamp Meter</h4>
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourceHasilClamp" :paginator="true"
            ...
            <h4>Meter</h4>
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourceHasilMeter" :paginator="true"
              :rows="10" :rowsPerPageOptions="[5, 10, 25]" :loading="isLoading" showGridlines responsiveLayout="stack"
              breakpoint="960px" currentPageReportTemplate="Showing {first} to {last} of {totalRecords}">
              <ColumnGroup type="header">
                <Row>
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`manager/get-lembar-kerja-clamp-meter?norecdetail=${NOREC_DETAIL}`);
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`manager/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilClamp.value[0].norecregis}&norec_detail=${dataSourceHasilClamp.value[0].detailregistraifk}`);
}
