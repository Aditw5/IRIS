      ...
      <TabPanel>
        ...
        <VCard>
          <div class="column is-12">
            ...
            <h4>Set Point</h4>
            <DataTable :value="dataSourceHasilSetpoint" dataKey="norec" rowGroupMode="rowspan" groupRowsBy="group"
            ...
            <h4>Rentang</h4>
            <DataTable :value="dataSourceHasilRentang" dataKey="no" rowGroupMode="rowspan" groupRowsBy="group"
              :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
              class="p-datatable-customers p-datatable-sm" filterDisplay="menu"
              paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown"
              responsiveLayout="stack" breakpoint="960px" sortMode="multiple" showGridlines
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`manager/get-lembar-kerja-thermocouple-rtd-simulator?norecdetail=${NOREC_DETAIL}`);
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`manager/cetak-sertifikat-lembar-kerja?pdf=true&norec=${firstResult().norecregis}&norec_detail=${firstResult().detailregistraifk}`);
}
