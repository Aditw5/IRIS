      ...
      <TabPanel>
        ...
        <VCard>
          <div class="column is-12">
            ...
            <DataTable :value="dataSourceHasilKalibrasi" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
            ...
            <DataTable :value="dataSourceHasilKarakteristik" :paginator="true" :rows="10"
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`manager/get-lembar-kerja-dryblock?norecdetail=${NOREC_DETAIL}`);
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`manager/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilKalibrasi.value[0].norecregis}&norec_detail=${dataSourceHasilKalibrasi.value[0].detailregistraifk}`);
}
