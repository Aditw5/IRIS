      ...
      <TabPanel>
        ...
        <VCard>
          <div class="column is-12">
            ...
            <h4>Sumber</h4>
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourceHasilSumber" :paginator="true"
              :rows="10" :rowsPerPageOptions="[5, 10, 25]" :loading="isLoading" showGridlines responsiveLayout="stack"
              breakpoint="960px" currentPageReportTemplate="Showing {first} to {last} of {totalRecords}">
              <ColumnGroup type="header">
                <Row>
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`manager/get-lembar-kerja-meter-sumber?norecdetail=${NOREC_DETAIL}`);
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`manager/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilMeter.value[0].norecregis}&norec_detail=${dataSourceHasilMeter.value[0].detailregistraifk}`);
}
