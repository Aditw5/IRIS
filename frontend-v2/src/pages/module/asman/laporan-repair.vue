...
const savePenolakanLaporan = async () => {
  ...
  try {
    await useApi().post(`/asman/save-penolakan-laporan-repair`, json)
...
const setujuiLaporan = async () => {
  ...
  try {
    await useApi().post('/asman/save-setujui-laporan-repair', json)
...
const fetchData = async () => {
  ...
  try {
    const response: any = await useApi().get(`asman/get-laporan-repair?norecdetail=${NOREC_DETAIL}`)
...
const cetakLaporanRepair = () => {
  ...
  H.printBlade(`asman/cetak-laporan-repair?pdf=true&norec=${norecReg}&norec_detail=${NOREC_DETAIL}`)
}
