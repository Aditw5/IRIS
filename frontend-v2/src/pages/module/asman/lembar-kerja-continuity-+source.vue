...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`asman/get-lembar-kerja-continuity-source?norecdetail=${NOREC_DETAIL}`);
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`asman/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilContinuity.value[0].norecregis}&norec_detail=${dataSourceHasilContinuity.value[0].detailregistraifk}`);
}
