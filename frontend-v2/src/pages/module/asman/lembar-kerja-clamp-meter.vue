...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`asman/get-lembar-kerja-clamp-meter?norecdetail=${NOREC_DETAIL}`);
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`asman/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilClamp.value[0].norecregis}&norec_detail=${dataSourceHasilClamp.value[0].detailregistraifk}`);
}
