...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`asman/get-lembar-kerja-dryblock?norecdetail=${NOREC_DETAIL}`);
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`asman/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilKalibrasi.value[0].norecregis}&norec_detail=${dataSourceHasilKalibrasi.value[0].detailregistraifk}`);
}
