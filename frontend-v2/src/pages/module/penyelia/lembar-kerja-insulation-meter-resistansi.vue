        ...
        <VCard>
          ...
          <div class="column is-12" v-else>
            ...
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourcefilter" :paginator="true" :rows="10"
              ...
              <Column field="penunjukan_standar_2" header="Penunjukan Standar 2" />
              <Column field="penunjukan_standar_satuan_2" header="" />
...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-insulation-meter-resistansi?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-insulation-meter-resistansi?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      if (currentGroup && String(row[0] ?? '').trim() !== '') {
        ...
        currentGroup.data.push({
          ...
          penunjukan_standar_2: rowData[4] ?? '',
          penunjukan_standar_satuan_2: rowData[5] ?? '',
          pembacaan_alat: rowData[6] ?? '',
          pembacaan_alat_satuan: rowData[7] ?? '',
          koreksi: rowData[8] ?? '',
          koreksi_satuan: rowData[9] ?? '',
          ketidakpastian: rowData[10] ?? '',
          ketidakpastian_standar: rowData[11] ?? ''
        });
      }
    }
...
const Save = async () => {
  ...
  const rows = (dataSourcefilter.value || [])
    ...
    .map((r: any) => ({
      ...
      penunjukan_standar_2: toStr(r?.penunjukan_standar_2),
      penunjukan_standar_satuan_2: toStr(r?.penunjukan_standar_satuan_2),
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/penyelia/save-data-upload-lembar-kerja-insulation-meter-resistansi', formData)
    isLoadingSave.value = false
