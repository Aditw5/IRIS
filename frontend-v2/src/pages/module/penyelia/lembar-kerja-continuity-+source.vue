...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-continuity-source?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`penyelia/get-lembar-kerja-continuity-source?norecdetail=${NOREC_DETAIL}`);
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/penyelia/save-data-upload-lembar-kerja-continuity-source`, json
  ).then((response: any) => {
    isLoadingSave.value = false;
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`penyelia/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilContinuity.value[0].norecregis}&norec_detail=${dataSourceHasilContinuity.value[0].detailregistraifk}`);
}
