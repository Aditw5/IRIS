    ...
    <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Caliper</span>
        ...
        <VCard>
          ...
          <div class="column is-12" v-else>
            ...
            <div class="column is-4 mt-5-min">
              <div class="column is-12">
                <span>Pengukuran Kesejajaran Luar </span>
              </div>
              <VField addons>
                <VControl>
                  <VInput v-model="item.kesejajaranluar" placeholder="engukuran Kesejajaran Luar " />
                </VControl>
                <VControl class="field-addon-body">
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Caliper </h3>
              </div>
            </div>
...
useHead({
  title: 'Lembar Kerja Caliper - ' + import.meta.env.VITE_PROJECT,
})
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-caliper-micrometer?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-caliper-micrometer?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const onSelectedFiles = async (event) => {
  ...
  reader.onload = (e) => {
    ...
    // raw:false agar angka bertahan sesuai tampilan (mis. koma desimal)
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      // Lewati baris header (setelah judul grup)
      if (skipHeaderRows > 0) {
        skipHeaderRows--;
        continue;
      }

      // Deteksi judul grup: hanya kolom A berisi teks, dan baris berikutnya terlihat seperti header tabel
      if (nonEmptyCount === 1 && typeof row[0] === 'string' && looksLikeHeader(jsonData[i + 1])) {
        currentGroup = { group: row[0], data: [] };
        groups.push(currentGroup);
        nomor = 1;
        skipHeaderRows = 2; // selalu 2 baris header setelah judul grup
        continue;
      }

      // Baris data: cukup kolom A terisi, kolom lain boleh kosong
    ...
    // Flatten: setiap baris membawa field `group`
    const flatData = [];
...
const Save = async () => {
  ...
  if (!item.value.kesejajaranluar) { H.alert('warning', 'Kesejajaran Luar harus di isi'); return }
  ...
  formData.append('kesejajaranluar', item.value.kesejajaranluar)
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/penyelia/save-data-upload-lembar-kerja-caliper-micrometer', formData)
    isLoadingSave.value = false
...
const detailOrder = async () => {
  ...
  item.value.kesejajaranluar = data.kesejajaranluar;
