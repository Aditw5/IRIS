...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-tachometer?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-tachometer?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const Save = async () => {
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/penyelia/save-data-upload-lembar-kerja-tachometer', formData)
    isLoadingSave.value = false
