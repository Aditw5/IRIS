...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-sumber?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-sumber?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/penyelia/save-data-upload-lembar-kerja-sumber`, json).then((response: any) => {
      isLoadingSave.value = false
