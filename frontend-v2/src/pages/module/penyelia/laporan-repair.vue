          ...
          <div class="column is-12">
            ...
            <div class="columns is-multiline">
              <div class="column is-12 is-flex is-justify-content-flex-end mt-4">
                <VButton icon="feather:save" @click="Save()" :loading="isLoadingSave" color="info">
                  Simpan
                </VButton>
              </div>
            </div>
      ...
      <form class="modal-form">
        <div class="columns is-multiline">
          <div class="column is-12" v-if="editType === 'status'">
            <VField>
              <label class="form-label">Bagian Alat</label>
              <VControl>
          ...
          <div class="column is-12" v-if="editType === 'laporan'">
            <VField>
              <label class="form-label">Sparepart & Material Consumable</label>
              <VControl>
          ...
          <div class="column is-12" v-if="editType === 'hasil'">
            <VField>
              <label class="form-label">Hasil Repair</label>
              <VControl>
...
const hasPrintableData = computed(() => totalData.value > 0)
...
const pushFotoToTarget = (
  ...
  if (type === 'status') {
    ...
    if (isFileAlreadyExists(fileMitraStatus.value[index], file)) return
  ...
  if (type === 'repair') {
    ...
    if (isFileAlreadyExists(fileMitraRepair.value[index], file)) return
  ...
  if (type === 'hasil') {
    ...
    if (isFileAlreadyExists(fileMitraHasil.value[index], file)) return
...
const fetchData = async () => {
  ...
  await useApi()
    .get(`penyelia/get-laporan-repair?norecdetail=${NOREC_DETAIL}`)
    .then((response: any) => {
...
const detailOrder = async () => {
  ...
  if (!data) {
    ...
    H.alert('warning', 'Laporan repair masih draft pelaksana dan belum masuk antrian penyelia')
    toDashboard()
    return
  }
...
const Save = async () => {
  if (!item.value.statusrepair) {
    H.alert('warning', 'Status Keberhasilan Repair harus diisi')
    return
  }
  ...
  if (totalFilledRows === 0) {
    H.alert('warning', 'Minimal isi salah satu bagian: Status Alat, Tindakan Teknis, atau Hasil Repair')
    return
  }

  if (!validateStatusAlatFilledRows(filledStatusRows)) return
  if (!validateTindakanTeknisFilledRows(filledRepairRows)) return
  if (!validateHasilRepairFilledRows(filledHasilRows)) return
  ...
  try {
    ...
    H.alert('success', 'Laporan repair berhasil disimpan')
    clear()
    await fetchData()
    await detailOrder()
  } catch (e: any) {
...
const saveEditData = async () => {
  ...
  if (editType.value === 'status') {
    endpoint = '/penyelia/update-status-repair'
  }

  if (editType.value === 'laporan') {
    endpoint = '/penyelia/update-laporan-repair'
  }

  if (editType.value === 'hasil') {
    endpoint = '/penyelia/update-hasil-repair'
  }

  if (editType.value === 'kesimpulan') {
    endpoint = '/penyelia/update-kesimpulan-repair'
  }
...
const hapusStatus = async (e: any) => {
  ...
  try {
    await useApi().post(`penyelia/hapus-status-repair`, { norec: e.norec })
...
const hapusLaporan = async (e: any) => {
  ...
  try {
    await useApi().post(`penyelia/hapus-laporan-repair`, { norec: e.norec })
...
const hapusHasil = async (e: any) => {
  ...
  try {
    await useApi().post(`penyelia/hapus-hasil-repair`, { norec: e.norec })
...
const setujuiLaporan = async () => {
  ...
  try {
    await useApi().post('/penyelia/save-setujui-laporan-repair', json)
    ...
    if (error.response) {
      H.alert('error', `Kesalahan: ${error.response.status} - ${error.response.data.message || 'Gagal menyetujui laporan'}`)
...
const savePenolakanLaporan = async () => {
  if (!item.value.alasanpenolakan) {
    H.alert('warning', 'Alasan Penolakan harus diisi')
    return
  }
  ...
  try {
    await useApi().post(`/penyelia/save-penolakan-laporan-repair`, json)
    H.alert('success', 'Penolakan laporan berhasil disimpan')
    modalPenolakanLapoaran.value = false
    toDashboard()
  } catch (e: any) {
    console.error('Gagal simpan penolakan:', e)
...
const cetakLaporanRepair = () => {
  ...
  H.printBlade(`penyelia/cetak-laporan-repair?pdf=true&norec=${norecReg}&norec_detail=${NOREC_DETAIL}`)
}
