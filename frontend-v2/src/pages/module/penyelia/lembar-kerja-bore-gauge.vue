        ...
        <VCard>
          ...
          <div class="column is-12" v-else>
            ...
            <div class="columns is-multiline">
              ...
              <div class="column is-12">
                <Fieldset legend="- Spesifikasi Dial Indicator" :toggleable="true">
                  <div class="columns is-multiline">
                    <div class="column is-2 mt-5-min">
                      <div class="column is-12">
                        <span>Merek Pabrik </span>
                      </div>
                      <VField>
                        <VControl>
                          <VInput v-model="item.merekdial" placeholder="Merek Pabrik" />
                        </VControl>
                      </VField>
                    </div>
                    <div class="column is-2 mt-5-min">
                      ...
                      <VField>
                        <VControl>
                          <VInput v-model="item.tipedial" placeholder="Tipe" />
                        </VControl>
                      </VField>
                    </div>
                    <div class="column is-2 mt-5-min">
                      ...
                      <VField>
                        <VControl>
                          <VInput v-model="item.sndial" placeholder="SN" />
                        </VControl>
                      </VField>
                    </div>
                    <div class="column is-3 mt-5-min">
                      ...
                      <VField addons>
                        <VControl>
                          <VInput v-model="item.rangedial" placeholder="Range" />
                        </VControl>
                        <VControl class="field-addon-body">
                          <VButton static>mm</VButton>
                        </VControl>
                    ...
                    <div class="column is-3 mt-5-min">
                      ...
                      <VField addons>
                        <VControl>
                          <VInput v-model="item.resolusidial" placeholder="Resolusi" />
                        </VControl>
                        <VControl class="field-addon-body">
                          <VButton static>mm</VButton>
                        </VControl>
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-bore-gauge?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-bore-gauge?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const onSelectedFiles = async (event) => {
  ...
  reader.onload = (e) => {
    ...
    const COL_PAT = {
      ...
      penunjukan_standar: /(penunjukan.*standar|calibrator.*output)/i,
      koreksi: /(koreksi|correction)/i,
      ketidakpastian: /(ketidakpastian|uncertainty)/i,
    };
    const SUBHDR_PAT = /(instrument.*reading|calibrator.*output|correction|uncertainty|mm)/i;
    ...
    const { start, map } = headerInfo;

    const ambil = (row, k) =>
      map[k] !== undefined ? String(row[map[k]] ?? '').trim() : '';
    const satuan = (row, k) =>
      map[k] !== undefined ? String(row[map[k] + 1] ?? '').trim() : '';
    ...
    for (let i = start; i < rows.length; i++) {
      ...
      hasil.push({
        ...
        penunjukan_standar: ambil(row, 'penunjukan_standar'),
        penunjukan_standar_satuan: satuan(row, 'penunjukan_standar'),
        koreksi: ambil(row, 'koreksi'),
        koreksi_satuan: satuan(row, 'koreksi'),
...
const Save = async () => {
  ...
  if (!item.value.merekdial) { H.alert('warning', 'Merek Dial harus di isi'); return }
  if (!item.value.tipedial) { H.alert('warning', 'Tipe Dial harus di isi'); return }
  if (!item.value.sndial) { H.alert('warning', 'Serial Number Dial harus di isi'); return }
  if (!item.value.rangedial) { H.alert('warning', 'Range Dial harus di isi'); return }
  if (!item.value.resolusidial) { H.alert('warning', 'Resolusi Dial harus di isi'); return }
  ...
  formData.append('merekdial', item.value.merekdial)
  formData.append('tipedial', item.value.tipedial)
  formData.append('sndial', item.value.sndial)
  formData.append('rangedial', item.value.rangedial)
  formData.append('resolusidial', item.value.resolusidial)
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/penyelia/save-data-upload-lembar-kerja-bore-gauge', formData)
    isLoadingSave.value = false
...
const detailOrder = async () => {
  ...
  item.value.merekdial = data.merekdiallembarkerja
  item.value.tipedial = data.tipediallembarkerja
  item.value.sndial = data.serialnumberdiallembarkerja
  item.value.rangedial = data.rangediallembarkerja
  item.value.resolusidial = data.resolusidiallembarkerja
