            ...
            <DataTable v-model:editingRows="editingRows" :value="dataSourceHasilKalibrasi" editMode="row" dataKey="no"
              ...
              <template #header>
                <div class="columns is-multiline is-vcentered is-mobile pt-0 pb-0" style="gap: 0.25rem;">
                  <div class="column is-narrow" v-if="dataSourceHasilKalibrasi.length > 0">
                  ...
                  <div class="column is-narrow" v-if="dataSourceHasilKalibrasi.length > 0">
                    <VButton color="danger" outlined icon="feather:trash" @click="batalRegis(dataSourceHasilKalibrasi)">
                      Tolak Sertifikat
                    </VButton>
                  </div>
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-dryblock?token=' + useUserSession().token, '_blank');
}
...
const setujuiSertifikat = async () => {
  let json = {
    'verif': {
      'norec': dataSourceHasilKalibrasi.value[0].detailregistraifk ?? '',
    }
  }
  isLoadingSave.value = true
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`penyelia/get-lembar-kerja-dryblock?norecdetail=${NOREC_DETAIL}`);
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          if (!isKalibrasi && /instrument reading/i.test(row.join(' '))) {
            isKalibrasi = true;
            headerRowCount = 0;
            continue;
          }
          if (!isKarakteristik && /kedalaman pencelupan/i.test(row.join(' '))) {
            isKalibrasi = false;
            isKarakteristik = true;
            headerRowCount = 0;
            continue;
          }
          if ((isKalibrasi || isKarakteristik) && headerRowCount < 2 && row.some(cell => typeof cell === "string")) {
            headerRowCount++;
            continue;
          }
...
const Save = async () => {
  ...
  formData.append('tglkalibrasi', item.value.tglkalibrasi)
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/penyelia/save-data-upload-lembar-kerja-dryblock`, formData
  ).then((response: any) => {
    isLoadingSave.value = false;
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`penyelia/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilKalibrasi.value[0].norecregis}&norec_detail=${dataSourceHasilKalibrasi.value[0].detailregistraifk}`);
}
