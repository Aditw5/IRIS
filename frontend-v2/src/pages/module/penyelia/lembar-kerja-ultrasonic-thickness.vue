    ...
    <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Ultrasonic Thickness</span>
        ...
        <VCard>
          ...
          <div class="column is-12" v-else>
            ...
            <div class="columns is-multiline">
              ...
              <div class="column is-3 mt-5-min">
                <div class="column is-12">
                  <span>Standar Material </span>
                </div>
                <VField>
                  <VControl>
                    <VInput v-model="item.standarmaterial" placeholder="Standar Material" />
                  </VControl>
                </VField>
              </div>
              <div class="column is-3 mt-5-min">
                ...
                <VField addons>
                  <VControl>
                    <VInput v-model="item.kecepatan" placeholder="Kecepatan" />
                  </VControl>
                  <VControl class="field-addon-body">
                    <VButton static>m/s</VButton>
                  </VControl>
                </VField>
              </div>
            </div>
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Ultrasonic Thickness </h3>
              </div>
            </div>
...
useHead({
  title: 'Lembar Kerja Ultrasonic Thickness - ' + import.meta.env.VITE_PROJECT,
})
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-ultrasonic-thickness?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-ultrasonic-thickness?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const onSelectedFiles = async (event: { files: any; }) => {
  ...
  reader.onload = (e) => {
    ...
    const HEADER_RE = /rentang|range|penunjukan|calibrator|pembacaan|instrument|koreksi|correction|ketidakpastian|uncertainty/i;
    const looksLikeHeader = (r: any) =>
      Array.isArray(r) && r.some(c => typeof c === 'string' && HEADER_RE.test(c ?? ''));

    let groups: Array<{ group: string; data: any[] }> = [];
    let currentGroup: { group: string; data: any[] } | null = null;
    let nomor = 1;
    let skipHeaderRows = 0;

    for (let i = 0; i < jsonData.length; i++) {
      const row = (jsonData[i] || []) as any[];
      const nonEmptyCount = row.filter(v => v !== undefined && v !== null && String(v).trim() !== '').length;
      ...
      if (nonEmptyCount === 1 && typeof row[0] === 'string' && looksLikeHeader(jsonData[i + 1])) {
      ...
      if (currentGroup && String(row[0] ?? '').trim() !== '') {
        const rowData = row as any[];
        currentGroup.data.push({
          no: nomor++,
          rentang: rowData[0] ?? '',
          rentang_satuan: rowData[1] ?? '',
          penunjukan_standar: rowData[2] ?? '',
          penunjukan_standar_satuan: rowData[3] ?? '',
          pembacaan_alat: rowData[4] ?? '',
          pembacaan_alat_satuan: rowData[5] ?? '',
          koreksi: rowData[6] ?? '',
          koreksi_satuan: rowData[7] ?? '',
          ketidakpastian: rowData[8] ?? '',
          ketidakpastian_standar: rowData[9] ?? ''
        });
      }
    }
...
const dataSourcefilter = computed(() => {
  let tarifTotal = 0;
  const filteredData = dataSource.value.filter(() => {
    return (
      (!filterd.value)
    );
  });
  tarifTotal = filteredData.reduce((total: number, data: { TOTAL_TARIF: string; }) => total + parseFloat(data.TOTAL_TARIF), 0);
...
const Save = async () => {
  ...
  if (!item.value.resolusi) { H.alert('warning', 'Resolution harus di isi'); return }
  ...
  if (!item.value.standarmaterial) { H.alert('warning', 'Standar Material harus di isi'); return }
  if (!item.value.kecepatan) { H.alert('warning', 'Kecepatan harus di isi'); return }
  ...
  const rows = (dataSourcefilter.value || [])
    .filter((r: any) => !r?.isGroupHeader && toStr(r?.rentang) !== '')
    .map((r: any) => ({
      no: r?.no ?? '',
      group: toStr(r?.group),
  ...
  formData.append('standarmaterial', item.value.standarmaterial)
  formData.append('kecepatan', item.value.kecepatan)
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/penyelia/save-data-upload-lembar-kerja-ultrasonic-thickness', formData)
    isLoadingSave.value = false
...
const detailOrder = async () => {
  ...
  item.value.kecepatan = data.kecepatan
  item.value.standarmaterial = data.standarmaterial
  ...
  item.value.detailInstruksiKerja = (data.daftarinstruksikerja?.length > 0)
    ? data.daftarinstruksikerja.map((i: { value: any; label: any; }) => ({
  ...
  item.value.detailPeralatanStandar = (data.daftaralatstandar?.length > 0)
    ? data.daftaralatstandar.map((a: { value: any; namaalatstandar: any; namamerk: any; namatipe: any; namaserialnumber: any; }) => ({
      daftaralatstandar: {
        value: a.value ?? '',
