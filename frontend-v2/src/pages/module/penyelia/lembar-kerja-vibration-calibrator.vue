            ...
            <DataTable v-model:editingRows="editingRowsVSM" :value="dataSourceHasilVSM" editMode="row" dataKey="no"
              ...
              <template #header>
                <div class="columns is-multiline is-vcentered is-mobile pt-0 pb-0" style="gap: 0.25rem;">
                  ...
                  <div class="column is-narrow" v-if="dataSourceHasilAccelerometer.length > 0">
                    <VButton color="danger" outlined icon="feather:trash" @click="batalRegis(dataSourceAccelerometer)">
                      Tolak Sertifikat
                    </VButton>
                  </div>
                  <div class="column is-narrow" v-if="dataSourceHasilAccelerometer.length > 0">
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`penyelia/get-lembar-kerja-vibration-calibrator?norecdetail=${NOREC_DETAIL}`);
...
const Save = async () => {
  ...
  isLoadingSave.value = true
  await useApi().post(`/penyelia/save-data-upload-lembar-kerja-vibration-calibrator`, payload)
    .then(() => {
      isLoadingSave.value = false
...
const cetakSertifikatLembarKerja = () => {
  ...
  H.printBlade(`penyelia/cetak-sertifikat-lembar-kerja?pdf=true&norec=${src[0].norecregis}&norec_detail=${src[0].detailregistraifk}`)
}
