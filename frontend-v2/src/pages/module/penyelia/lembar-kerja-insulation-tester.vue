    ...
    <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Insulation Tester</span>
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Insulation Tester </h3>
              </div>
            </div>
...
useHead({
  title: 'Lembar Kerja Insulation Tester - ' + import.meta.env.VITE_PROJECT,
})
...
const downloadTemplate = () => {
  window.open('/service/pelaksana/download-template-lembar-kerja-insulation-tester?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-insulation-tester?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const Save = async () => {
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/penyelia/save-data-upload-lembar-kerja-insulation-tester', formData)
    isLoadingSave.value = false
