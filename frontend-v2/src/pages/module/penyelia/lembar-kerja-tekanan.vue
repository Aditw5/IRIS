...
const onRowEditSave = async (event: any) => {
  ...
  try {
    await useApi().post(`/penyelia/edit-lembar-kerja`, payload)
  } catch (err: any) {
    console.error(err)
  } finally {
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-tekanan?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-tekanan?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const Save = async () => {
  ...
  isLoadingSave.value = true
  await useApi()
    .post(`/penyelia/save-data-upload-lembar-kerja-tekanan`, json)
    .then(() => {
      isLoadingSave.value = false
