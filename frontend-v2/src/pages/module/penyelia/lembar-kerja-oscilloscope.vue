    ...
    <TabView class="tabview-custom " :scrollable="true">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Oscilloscope</span>
        ...
        <VCard>
          ...
          <div class="column is-12" v-else>
            ...
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourcefilter" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
              ...
              <ColumnGroup type="header">
                <Row>
                  <Column header="No" />
                  <Column header="group" />
                  <Column header="Skala Instrumen" :colspan="2" />
                  <Column header="Indikasi Standar" :colspan="2" />
                  <Column header="Indikasi Instrumen" :colspan="2" />
              ...
              <Column field="skala_instrumen" header="Skala Instrumen" />
              <Column field="skala_instrumen_satuan" header="" />
              <Column field="indikasi_standar" header="Indikasi Standar" />
              <Column field="indikasi_standar_satuan" header="" />
              <Column field="indikasi_instrumen" header="Indikasi Instrumen" />
              <Column field="indikasi_instrumen_satuan" header="" />
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Oscilloscope</h3>
              </div>
            </div>
...
useHead({
  title: 'Lembar Kerja Oscilloscope - ' + import.meta.env.VITE_PROJECT,
})
...
const normalizeOscilloscopeRow = (row: any) => ({
  ...(row?.norec ? { norec: row.norec } : {}),
  isGroupHeader: row?.isGroupHeader === true,
  group: toStr(row?.group),
  no: row?.no ?? '',
  skala_instrumen: toStr(row?.skala_instrumen ?? row?.rentang),
  skala_instrumen_satuan: toStr(row?.skala_instrumen_satuan ?? row?.rentang_satuan),
  indikasi_standar: toStr(row?.indikasi_standar ?? row?.penunjukan_standar),
  indikasi_standar_satuan: toStr(row?.indikasi_standar_satuan ?? row?.penunjukan_standar_satuan),
  indikasi_instrumen: toStr(row?.indikasi_instrumen ?? row?.pembacaan_alat),
  indikasi_instrumen_satuan: toStr(row?.indikasi_instrumen_satuan ?? row?.pembacaan_alat_satuan),
  koreksi: toStr(row?.koreksi),
  koreksi_satuan: toStr(row?.koreksi_satuan),
  ketidakpastian: toStr(row?.ketidakpastian),
  ketidakpastian_standar: toStr(row?.ketidakpastian_standar),
})
...
const setujuiSertifikat = async () => {
  ...
  await useApi().post('/penyelia/save-setujui-serti', json).then(() => {
...
const onRowEditSave = async (event: any) => {
  ...
  const rowPayload = normalizeOscilloscopeRow(newData)
  dataSourceHasilLembarKerja.value[index] = {
    ...newData,
    ...rowPayload,
  }
  const payload = {
...
async function onScanSuccess(decodedText: string) {
  ...
  const { data } = await useApi().get(
    `penyelia/alat-standar?id_alat=${id_alat}`
  )
  if (!data.length) {
    scannerItem.value = null
    return
...
const fetchAlatStandar = async (filter: any) => {
  await useApi().get(
    `penyelia/alat-standar?param_search=namaalatstandar,namamerk,namatipe,namaserialnumber&query=${filter.query}`
  ).then((response) => {
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-oscilloscope?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-oscilloscope?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
    response.forEach((element: any, i: any) => {
      element.no = i + 1
      Object.assign(element, normalizeOscilloscopeRow(element))
    });
...
const onSelectedFiles = async (event: any) => {
  ...
  reader.onload = (e: any) => {
    ...
    const HEADER_RE =
      /skala\s*instrumen|indikasi\s*standar|indikasi\s*instrumen|instrument\s*scale|standard\s*indication|instrument\s*indication|koreksi|correction|ketidakpastian|uncertainty/i
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      if (nonEmptyCount === 1 && typeof row[0] === 'string' && toStr(row[0]).length > 2) {
        currentGroup = {
          group: toStr(row[0]),
          data: []
        }
      ...
      if (currentGroup && headerRowCount < 2 && row.some((cell: any) => typeof cell === 'string')) {
        headerRowCount++
      ...
      if ((currentGroup || sudahMasukAreaData) && toStr(row[0]) !== '') {
        const dataRow = {
          no: nomor++,
          skala_instrumen: toStr(row[0]),
          skala_instrumen_satuan: toStr(row[1]),
          indikasi_standar: toStr(row[2]),
          indikasi_standar_satuan: toStr(row[3]),
          indikasi_instrumen: toStr(row[4]),
          indikasi_instrumen_satuan: toStr(row[5]),
          koreksi: toStr(row[6]),
        ...
        if (currentGroup) {
          currentGroup.data.push(dataRow)
        } else {
          flatData.push(dataRow)
        }
      }
    }

    dataSource.value = groups.length > 0
    ...
    if (dataSource.value.length === 0) {
      H.alert(
        'warning',
        'Data Excel tidak terbaca. Pastikan template berisi group, header Skala Instrumen, dan data mulai di bawah header.'
      )
    }

    console.log('DATA EXCEL TERBACA:', dataSource.value)
  }
...
const Save = async () => {
  ...
  const rows = (dataSourcefilter.value || [])
    .map((r: any) => normalizeOscilloscopeRow(r))
    .filter((r: any) => toStr(r?.skala_instrumen) !== '')
  ...
  formData.append('fileName', item.value.fileName ?? '')
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/penyelia/save-data-upload-lembar-kerja-oscilloscope', formData)
    isLoadingSave.value = false
