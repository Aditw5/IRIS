    ...
    <TabView class="tabview-custom " :scrollable="true">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Insulation Source</span>
        ...
        <VCard>
          ...
          <div class="column is-12" v-else>
            ...
            <h4>Insulation</h4>
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourceInsulation" :paginator="true"
            ...
            <h4>Source</h4>
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourceSource" :paginator="true" :rows="10"
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Insulation Source </h3>
              </div>
            </div>
            <h4>Insulation</h4>
            <DataTable v-model:editingRows="editingRows" :value="dataSourceHasilInsulation" editMode="row" dataKey="no"
              ...
              <template #header>
                <div class="columns is-multiline is-vcentered is-mobile pt-0 pb-0" style="gap: 0.25rem;">
                  <div class="column is-narrow" v-if="dataSourceHasilInsulation.length > 0">
                  ...
                  <div class="column is-narrow" v-if="dataSourceHasilInsulation.length > 0">
                    <VButton color="danger" outlined icon="feather:trash"
                      @click="batalRegis(dataSourceHasilInsulation)">
                      Tolak Sertifikat
                    </VButton>
                  </div>
                  <div class="column is-narrow" v-if="dataSourceHasilInsulation.length > 0">
            ...
            <h4>Source</h4>
            <DataTable v-model:editingRows="editingRowsSource" :value="dataSourceHasilSource" editMode="row"
              dataKey="no" @row-edit-save="onRowEditSaveSource" rowGroupMode="rowspan" groupRowsBy="group"
              ...
              <template #header>
                <div class="columns is-multiline is-vcentered is-mobile pt-0 pb-0" style="gap: 0.25rem;">
                  <div class="column is-narrow" v-if="dataSourceHasilSource.length > 0">
                    <VButton color="primary" @click="cetakSertifikatLembarKerja()" outlined icon="feather:printer">
                      Cetak Sertifikat
                    </VButton>
                  </div>
                  ...
                  <div class="column is-narrow" v-if="dataSourceHasilSource.length > 0">
                    <VButton color="danger" outlined icon="feather:trash" @click="batalRegis(dataSourceHasilSource)">
                      Tolak Sertifikat
                    </VButton>
                  </div>
                  <div class="column is-narrow" v-if="dataSourceHasilSource.length > 0">
...
useHead({
  title: 'Lembar Kerja Insulation Source - ' + import.meta.env.VITE_PROJECT,
})
...
const dataSourceSource: any = ref([])
const dataSourceInsulation: any = ref([])
const dataSourceHasilInsulation: any = ref([])
const dataSourceHasilSource: any = ref([])
const fileMitraExcel: any = ref()
const dataExcel: any = ref()
const editingRows = ref([]);
const editingRowsSource = ref([]);
const item: any = ref({
  filterTgl: reactive({
    start: new Date(),
    end: new Date(),
...
const setujuiSertifikat = async () => {
  let json = {
    'verif': {
      'norec': dataSourceHasilInsulation.value[0].detailregistraifk ?? '',
    }
  }
...
const onRowEditSave = async (event: any) => {
  ...
  dataSourceHasilInsulation.value[index] = newData
  const payload = {
    data: [newData],
...
const onRowEditSaveSource = async (event: any) => {
  const { newData, index } = event
  dataSourceHasilSource.value[index] = newData
  const payload = {
    data: [newData],
    sublingkupfk: SUBLINGKUP,
    norec_detail: NOREC_DETAIL
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-insulation-source?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`penyelia/get-lembar-kerja-insulation-source?norecdetail=${NOREC_DETAIL}`);
    response.forEach((element: any, i: any) => {
      element.no = i + 1
    });
    dataSourceHasilInsulation.value = response.filter((item: any) => item.jenis === 'insulation')
    dataSourceHasilSource.value = response.filter((item: any) => item.jenis === 'source')
  } catch (err) {
    dataSourceHasilInsulation.value = []
  ...
  if (files && files.length > 0) {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        const continuityGroups: typeof groups = [];
        const sourceGroups: typeof groups = [];

        let currentGroup: typeof continuityGroups[number] | null = null;
        let nomor = 1;
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          if (row.filter(x => !!x).length === 1 && typeof row[0] === 'string' && row[0].length > 2) {
            ...
            isInsulation = /calibrator output|effective current output/i.test((jsonData[i + 2] || []).join(' '));
            if (isInsulation) {
              continuityGroups.push(currentGroup);
            } else {
          ...
          if (currentGroup && headerRowCount === 2 && row[0] && row[2]) {
            ...
            if (isInsulation) {
              ...
              currentGroup.data.push({
                ...commonFields,
                penunjukan_uuc: row[2] ?? '',
                penunjukan_uuc_satuan: row[3] ?? '',
                pembacaan_standar: row[4] ?? '',
                pembacaan_standar_satuan: row[5] ?? '',
                ketidakpastian: row[8] ?? '',
                ketidakpastian_satuan: row[9] ?? ''
              });
        ...
        const flatContinuity = continuityGroups.flatMap((group: any) => [
          { isGroupHeader: true, group: group.group },
          ...group.data.map((row: any) => ({ ...row, group: group.group, isGroupHeader: false }))
        ]);

        const flatSource = sourceGroups.flatMap((group: any) => [
          { isGroupHeader: true, group: group.group },
          ...group.data.map((row: any) => ({ ...row, group: group.group, isGroupHeader: false }))
        ]);

        dataSourceInsulation.value = flatContinuity;
        dataSourceSource.value = flatSource;
      };
...
const onRemoveTemplatingFile = (file: any, removeFileCallback: any, index: any) => {
  ...
  dataSourceInsulation.value = [];
  dataSourceSource.value = [];
  valueProgress.value = 0;
};

const dataSourcefilter = computed(() => {
  return [...dataSourceInsulation.value, ...dataSourceSource.value];
});
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/penyelia/save-data-upload-lembar-kerja-insulation-source`, json
  ).then((response: any) => {
    isLoadingSave.value = false;
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`penyelia/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilInsulation.value[0].norecregis}&norec_detail=${dataSourceHasilInsulation.value[0].detailregistraifk}`);
}
