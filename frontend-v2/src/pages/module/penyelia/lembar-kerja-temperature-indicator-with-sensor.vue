...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-temperature-indicator?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-temperature-indicator?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/penyelia/save-data-upload-lembar-kerja-temperature-indicator`, json).then((response: any) => {
      isLoadingSave.value = false
