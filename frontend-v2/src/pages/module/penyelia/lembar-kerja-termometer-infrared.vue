    ...
    <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Termometer Infrared</span>
          ...
          <div class="column is-12" v-else>
            ...
            <div class="columns is-multiline">
              <div class="column is-3 mt-5-min">
                ...
                <VField addons>
                  <VControl>
                    <VInput v-model="item.resolusi" placeholder="range" />
                  </VControl>
              ...
              <div class="column is-3 mt-5-min">
                <div class="column is-12">
                  <span>Dikalibari Pada Jarak</span>
                </div>
                <VField addons>
                  <VControl>
                    <VInput v-model="item.jarakKalibrasi" placeholder="Jarak Kalibrasi" />
                  </VControl>
                  <VControl class="field-addon-body">
              ...
              <div class="column is-3 mt-5-min">
                <div class="column is-12">
                  <span>Emisivitas Black Body</span>
                </div>
                <VField>
                  <VControl>
                    <VInput v-model="item.emisivitasBlackBody" placeholder="Emisivitas Black Body" />
                  </VControl>
                </VField>
              </div>
            </div>
              ...
              <div class="column is-3 mt-4">
                <VCardCustom :style="'padding:5px 25px'">
                  <div class="label-status success">
                    ...
                    <span class="ml-1">TOTAL DATA TERMOMETER INFRARED</span>
                  </div>
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Termometer Infrared </h3>
              </div>
            </div>
...
useHead({
  title: 'Lembar Kerja Termometer Infrared - ' + import.meta.env.VITE_PROJECT,
})
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-thermometer-infrared?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-thermometer-infrared?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const Save = async () => {
  ...
  if (!item.value.rentangukursuhu) { H.alert('warning', 'Rentang Ukur Suhu harus di isi'); return }
  ...
  if (!item.value.jarakKalibrasi) { H.alert('warning', 'Jarak Kalibrasi harus di isi'); return }
  if (!item.value.emisivitasBlackBody) { H.alert('warning', 'Emisivitas Black Body harus di isi'); return }
  ...
  let json = {
    ...
    'jarakKalibrasi': item.value.jarakKalibrasi,
    'emisivitas': item.value.emisivitasBlackBody,
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/penyelia/save-data-upload-lembar-kerja-thermometer-infrared`, json).then((response: any) => {
      isLoadingSave.value = false
...
const detailOrder = async () => {
  ...
  item.value.jarakKalibrasi = data.jarakKalibrasi
  item.value.emisivitasBlackBody = data.emisivitas
