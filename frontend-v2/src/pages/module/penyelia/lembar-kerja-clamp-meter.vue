    ...
    <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Clamp & Meter</span>
        ...
        <VCard>
          ...
          <div class="column is-12" v-else>
              ...
              <div class="column is-3 mt-4">
                <VCardCustom :style="'padding:5px 25px'">
                  <div class="label-status success">
                    ...
                    <span class="ml-1">TOTAL DATA CLAMP & METER</span>
                  </div>
            ...
            <DataTable rowGroupMode="rowspan" groupRowsBy="group" :value="dataSourceClamp" :paginator="true" :rows="10"
              ...
              <Column field="penyetelan" />
              <Column field="penyetelan_satuan" />
              <Column field="keluaran" />
              <Column field="keluaran_satuan" />
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Clamp & Meter </h3>
              </div>
            </div>
            <h4>Clamp Meter</h4>
            <DataTable v-model:editingRows="editingRows" :value="dataSourceHasilClamp" editMode="row" dataKey="no"
              ...
              <template #header>
                <div class="columns is-multiline is-vcentered is-mobile pt-0 pb-0" style="gap: 0.25rem;">
                  <div class="column is-narrow" v-if="dataSourceHasilClamp.length > 0">
                  ...
                  <div class="column is-narrow" v-if="dataSourceHasilClamp.length > 0">
                    <VButton color="danger" outlined icon="feather:trash" @click="batalRegis(dataSourceHasilClamp)">
                      Tolak Sertifikat
                    </VButton>
                  </div>
            ...
            <h4>Meter</h4>
            <DataTable v-model:editingRows="editingRowsMeter" :value="dataSourceHasilMeter" editMode="row" dataKey="no"
              @row-edit-save="onRowEditSaveMeter" rowGroupMode="rowspan" groupRowsBy="group" :paginator="true"
...
useHead({
  title: 'Lembar Kerja Clamp & Meter - ' + import.meta.env.VITE_PROJECT,
})
...
const onRowEditSaveMeter = async (event: any) => {
  const { newData, index } = event
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-clamp-meter?token=' + useUserSession().token, '_blank');
}
...
const setujuiSertifikat = async () => {
  let json = {
    'verif': {
      'norec': dataSourceHasilClamp.value[0].detailregistraifk ?? '',
    }
  }
  isLoadingSave.value = true
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`penyelia/get-lembar-kerja-clamp-meter?norecdetail=${NOREC_DETAIL}`);
    response.forEach((element: any, i: any) => {
      element.no = i + 1
    });
    dataSourceHasilClamp.value = response.filter((item: any) => item.jenis === 'clamp')
    dataSourceHasilMeter.value = response.filter((item: any) => item.jenis === 'meter')
  } catch (err) {
  ...
  if (files && files.length > 0) {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        const clampGroups: typeof groups = [];
        const meterGroups: typeof groups = [];

        let currentGroup: typeof clampGroups[number] | null = null;
        let nomor = 1;
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          if (row.filter(x => !!x).length === 1 && typeof row[0] === 'string' && row[0].length > 2) {
            ...
            isClamp = /current source setting|effective current output/i.test((jsonData[i + 2] || []).join(' '));
            if (isClamp) {
              clampGroups.push(currentGroup);
            } else {
              meterGroups.push(currentGroup);
            }
            nomor = 1;
            headerRowCount = 0;
            continue;
          ...
          if (currentGroup && headerRowCount === 2 && row[0] && row[2]) {
            ...
            if (isClamp) {
              currentGroup.data.push({
                ...commonFields,
                penyetelan: row[2] ?? '',
                penyetelan_satuan: row[3] ?? '',
                keluaran: row[4] ?? '',
                keluaran_satuan: row[5] ?? '',
        ...
        const flatClamp = clampGroups.flatMap(group => [
          { isGroupHeader: true, group: group.group },
          ...group.data.map(row => ({ ...row, group: group.group, isGroupHeader: false }))
        ]);

        const flatMeter = meterGroups.flatMap(group => [
          { isGroupHeader: true, group: group.group },
          ...group.data.map(row => ({ ...row, group: group.group, isGroupHeader: false }))
        ]);

        dataSourceClamp.value = flatClamp;
        dataSourceMeter.value = flatMeter;
      };
...
const dataSourcefilter = computed(() => {
  return [...dataSourceClamp.value, ...dataSourceMeter.value];
});
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/penyelia/save-data-upload-lembar-kerja-clamp-meter`, json
  ).then((response: any) => {
    isLoadingSave.value = false;
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`penyelia/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilClamp.value[0].norecregis}&norec_detail=${dataSourceHasilClamp.value[0].detailregistraifk}`);
}
