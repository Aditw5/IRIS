    ...
    <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Dial Indicator</span>
            ...
            <DataTable :value="dataSourcefilter" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
              ...
              <ColumnGroup type="header">
                <Row>
                  ...
                  <Column :header="labelPenyimpanganPertama" :colspan="2" />
                  <Column :header="labelPenyimpanganKedua" :colspan="2" />
                  <Column header="Retrace Error" :colspan="2" />
              ...
              <Column field="penyimpangan_naik" :header="labelPenyimpanganPertama" />
              <Column field="penyimpangan_naik_satuan" header="" />
              <Column field="penyimpangan_turun" :header="labelPenyimpanganKedua" />
              <Column field="penyimpangan_turun_satuan" header="" />
              <Column field="retrace_error" header="Retrace Error" />
              <Column field="retrace_error_satuan" header="" />
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Dial Indicator </h3>
              </div>
            </div>
...
useHead({
  title: 'Lembar Kerja Dial Indicator - ' + import.meta.env.VITE_PROJECT,
})
...
const PENYIMPANGAN_LABELS = {
  naik_turun: {
    pertama: 'Penyimpangan Naik',
    kedua: 'Penyimpangan Turun',
  },
  maju_mundur: {
    pertama: 'Penyimpangan Maju',
    kedua: 'Penyimpangan Mundur',
  },
} as const
type JenisPenyimpangan = keyof typeof PENYIMPANGAN_LABELS
const normalizeJenisPenyimpangan = (value: any): JenisPenyimpangan =>
  value === 'maju_mundur' ? 'maju_mundur' : 'naik_turun'
const jenisPenyimpanganUpload = ref<JenisPenyimpangan>('naik_turun')
const jenisPenyimpanganAktif = computed<JenisPenyimpangan>(() => {
  const uploadRow = (dataSource.value || []).find((row: any) => row?.jenis_penyimpangan)
  if (uploadRow) return normalizeJenisPenyimpangan(uploadRow.jenis_penyimpangan)

  const savedRow = (dataSourceHasilLembarKerja.value || []).find((row: any) => row?.jenis_penyimpangan)
  if (savedRow) return normalizeJenisPenyimpangan(savedRow.jenis_penyimpangan)

  return jenisPenyimpanganUpload.value
})
const labelPenyimpanganPertama = computed(() => PENYIMPANGAN_LABELS[jenisPenyimpanganAktif.value].pertama)
const labelPenyimpanganKedua = computed(() => PENYIMPANGAN_LABELS[jenisPenyimpanganAktif.value].kedua)
const onUpload = () => {
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-dial-indicator?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-dial-indicator?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const onSelectedFiles = async (event) => {
  ...
  reader.onload = (e) => {
    ...
    const COL_PAT = {
      pembacaan_alat: /(pembacaan.*alat|instrument.*reading)/i,
      penyimpangan_naik: /(penyimpangan.*(naik|maju)|deviation.*(up|forward))/i,
      penyimpangan_turun: /(penyimpangan.*(turun|mundur)|deviation.*(down|backward))/i,
      retrace_error: /(retrace.*error|retrace)/i,
      ketidakpastian: /(ketidakpastian|uncertainty)/i,
    };
    const SUBHDR_PAT = /(instrument.*reading|deviation.*(up|down|forward|backward)|retrace.*error|uncertainty|mm)/i;
    type KolomDialIndicator = keyof typeof COL_PAT;
    const REQUIRED_KEYS = Object.keys(COL_PAT) as KolomDialIndicator[];

    const findHeader = () => {
      for (let r = 0; r < rows.length; r++) {
        const row = rows[r] || [];
        const map: Partial<Record<KolomDialIndicator, number>> = {};
        row.forEach((c, idx) => {
          const t = norm(c);
          REQUIRED_KEYS.forEach((k) => {
            if (map[k] === undefined && COL_PAT[k].test(t)) map[k] = idx;
          });
        });
        if (REQUIRED_KEYS.every((k) => map[k] !== undefined)) {
          let start = r + 1;
          const next = rows[r + 1] || [];
          const nextHasSub = next.some((c) => SUBHDR_PAT.test(norm(c)));
          if (nextHasSub) start = r + 2;
          const headerText = REQUIRED_KEYS.map((k) => norm(row[map[k] ?? -1])).join(' ');
          const jenisPenyimpangan = /(penyimpangan.*(maju|mundur)|\b(maju|mundur)\b)/i.test(headerText)
            ? 'maju_mundur'
            : 'naik_turun';
          return { start, map, jenisPenyimpangan };
        }
      }
      return null;
    };

    const headerInfo = findHeader();
    if (!headerInfo) return;

    const { start, map, jenisPenyimpangan } = headerInfo;
    jenisPenyimpanganUpload.value = normalizeJenisPenyimpangan(jenisPenyimpangan);

    const ambil = (row: any[], k: KolomDialIndicator) => {
      const idx = map[k];
      return idx !== undefined ? String(row[idx] ?? '').trim() : '';
    };
    const satuan = (row: any[], k: KolomDialIndicator) => {
      const idx = map[k];
      return idx !== undefined ? String(row[idx + 1] ?? '').trim() : '';
    };
    ...
    for (let i = start; i < rows.length; i++) {
      const row = rows[i] || [];
      const adaIsi = Object.values(map).some((idx) => String(row[idx] ?? '').trim() !== '');
      if (!adaIsi) continue;

      hasil.push({
        no: no++,
        jenis_penyimpangan: jenisPenyimpanganUpload.value,
        pembacaan_alat: ambil(row, 'pembacaan_alat'),
        pembacaan_alat_satuan: satuan(row, 'pembacaan_alat'),
        penyimpangan_naik: ambil(row, 'penyimpangan_naik'),
        penyimpangan_naik_satuan: satuan(row, 'penyimpangan_naik'),
        penyimpangan_turun: ambil(row, 'penyimpangan_turun'),
        penyimpangan_turun_satuan: satuan(row, 'penyimpangan_turun'),
        retrace_error: ambil(row, 'retrace_error'),
        retrace_error_satuan: satuan(row, 'retrace_error'),
        ketidakpastian: ambil(row, 'ketidakpastian'),
        ketidakpastian_satuan: satuan(row, 'ketidakpastian'),
      });
    }

    dataSource.value = hasil;
  };
...
const onRemoveTemplatingFile = (file: any, removeFileCallback: any, index: any) => {
  ...
  dataSource.value = []
  jenisPenyimpanganUpload.value = 'naik_turun'
...
const Save = async () => {
  ...
  const rows = (dataSourcefilter.value || [])
    .filter((r: any) => toStr(r?.pembacaan_alat) !== '')
    .map((r: any) => ({
      no: r?.no ?? '',
      jenis_penyimpangan: normalizeJenisPenyimpangan(r?.jenis_penyimpangan ?? jenisPenyimpanganAktif.value),
      pembacaan_alat: toStr(r?.pembacaan_alat),
      pembacaan_alat_satuan: toStr(r?.pembacaan_alat_satuan),

      penyimpangan_naik: toStr(r?.penyimpangan_naik),
      penyimpangan_naik_satuan: toStr(r?.penyimpangan_naik_satuan),

      penyimpangan_turun: toStr(r?.penyimpangan_turun),
      penyimpangan_turun_satuan: toStr(r?.penyimpangan_turun_satuan),

      retrace_error: toStr(r?.retrace_error),
      retrace_error_satuan: toStr(r?.retrace_error_satuan),

      ketidakpastian: toStr(r?.ketidakpastian),
      ketidakpastian_satuan: toStr(r?.ketidakpastian_satuan),
    }))
  ...
  formData.append('jenis_penyimpangan', jenisPenyimpanganAktif.value)
  ...
  try {
    isLoadingSave.value = true
    await useApi().post('/penyelia/save-data-upload-lembar-kerja-dial-indicator', formData)
    isLoadingSave.value = false
...
const detailOrder = async () => {
  ...
  item.value.gambarkelembaban = data.gambarkelembaban;
  item.value.resolusi = data.resolusi;
  item.value.range = data.rangelembarkerja;
  item.value.range_satuan = data.rangelembarkerjasatuan ?? 'mm';
