...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-meter-sumber?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`penyelia/get-lembar-kerja-meter-sumber?norecdetail=${NOREC_DETAIL}`);
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/penyelia/save-data-upload-lembar-kerja-meter-sumber`, json
  ).then((response: any) => {
    isLoadingSave.value = false;
