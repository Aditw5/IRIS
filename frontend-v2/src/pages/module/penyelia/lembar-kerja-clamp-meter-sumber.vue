...
const onRowEditSave = async (event: any) => {
  ...
  try { await useApi().post(`/penyelia/edit-lembar-kerja`, payload) }
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-clamp-meter-sumber?token=' + useUserSession().token, '_blank')
}
...
const fetchData = async () => {
  ...
  try {
    const response = await useApi().get(`penyelia/get-lembar-kerja-clamp-meter-sumber?norecdetail=${NOREC_DETAIL}`)
...
const Save = async () => {
  ...
  isLoadingSave.value = true
  await useApi().post(`/penyelia/save-data-upload-lembar-kerja-clamp-meter-sumber`, json)
    .then(() => {
      isLoadingSave.value = false
...
const cetakSertifikatLembarKerja = () => {
  H.printBlade(`penyelia/cetak-sertifikat-lembar-kerja?pdf=true&norec=${dataSourceHasilMeter.value[0].norecregis}&norec_detail=${dataSourceHasilMeter.value[0].detailregistraifk}`)
}
