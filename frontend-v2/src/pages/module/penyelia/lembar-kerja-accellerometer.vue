    ...
    <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Accellerometer</span>
        ...
        <VCard>
          ...
          <div class="column is-12" v-else>
              ...
              <div class="column is-3 mt-4">
                <VCardCustom :style="'padding:5px 25px'">
                  <div class="label-status success">
                    ...
                    <span class="ml-1">TOTAL DATA ACCELLEROMETER</span>
                  </div>
            ...
            <DataTable :value="dataSourcefilter" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
              ...
              <Column field="frekuensi" header="Frekuensi" />
              <Column field="frekuensi_satuan" header="" />
              <Column field="vibrasi_reference" header="Vibrasi Reference" />
              <Column field="vibrasi_reference_satuan" header="" />
              <Column field="uut" header="UUT" />
              <Column field="uut_satuan" header="" />
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Accellerometer </h3>
              </div>
            </div>
...
useHead({
  title: 'Lembar Kerja Accellerometer - ' + import.meta.env.VITE_PROJECT,
})
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-accellerometer?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-accellerometer?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        const rows: {
          no: number;
          frekuensi: any;
          frekuensi_satuan: any;
          vibrasi_reference: any;
          vibrasi_reference_satuan: any;
          uut: any;
          uut_satuan: any;
          ketidakpastian: any;
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          if (headerRowCount < 2 && row.some(cell => typeof cell === "string")) {
            headerRowCount++;
            continue;
          }
          if (headerRowCount === 2 && row[0] && row[2]) {
            rows.push({
              no: nomor++,
              frekuensi: row[0] ?? '',
              frekuensi_satuan: row[1] ?? '',
              vibrasi_reference: row[2] ?? '',
              vibrasi_reference_satuan: row[3] ?? '',
              uut: row[4] ?? '',
              uut_satuan: row[5] ?? '',
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/penyelia/save-data-upload-lembar-kerja-accellerometer`, json).then((response: any) => {
      isLoadingSave.value = false
