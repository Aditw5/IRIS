    ...
    <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
      <TabPanel>
        <template #header>
          ...
          <span>Lembar Kerja Sub Lingkup Thermohygrometer</span>
          ...
          <div class="column is-12" v-else>
            ...
            <div class="columns is-multiline">
              <div class="column is-3 mt-5-min">
                <div class="column is-12">
                  <span>Resolution °C</span>
                </div>
                <VField addons>
                  <VControl>
                    <VInput v-model="item.resolusi" placeholder="Resolution °C" />
                  </VControl>
              ...
              <div class="column is-3 mt-5-min">
                <div class="column is-12">
                  <span>Resolution %rH</span>
                </div>
                <VField addons>
                  <VControl>
                    <VInput v-model="item.resolusipersen" placeholder="Resolution %rH" />
                  </VControl>
                  <VControl class="field-addon-body">
              ...
              <div class="column is-3 mt-5-min">
                <div class="column is-12">
                  <span>Rentang Ukur %rH</span>
                </div>
                <VField addons>
                  <VControl>
                    <VInput v-model="item.rentangukursuhupersen" placeholder="Rentang Ukur %rH" />
                  </VControl>
                  <VControl class="field-addon-body">
                    <VButton static>%rH</VButton>
                  </VControl>
              ...
              <div class="column is-3 mt-5-min">
                <div class="column is-12">
                  <span>Kondisi Pengukuran Suhu </span>
                </div>
                <VField addons>
                  <VControl>
                    <VInput v-model="item.kondisiPengukuranSuhu" placeholder="Kondisi Pengukuran Suhu" />
                  </VControl>
                  <VControl class="field-addon-body">
                    <VButton static>% RH</VButton>
                  </VControl>
              ...
              <div class="column is-3 mt-5-min">
                <div class="column is-12">
                  <span>Kondisi Pengukuran Kelembaban </span>
                </div>
                <VField addons>
                  <VControl>
                    <VInput v-model="item.kondisiPengukuranKelembaban" placeholder="Kondisi Pengukuran Kelembaban" />
                  </VControl>
                  <VControl class="field-addon-body">
                    <VButton static>°C</VButton>
                  </VControl>
              ...
              <div class="column is-3 mt-4">
                <VCardCustom :style="'padding:5px 25px'">
                  <div class="label-status success">
                    ...
                    <span class="ml-1">TOTAL DATA THERMOHYGROMETER</span>
                  </div>
        ...
        <VCard>
          <div class="column is-12">
            ...
            <div class="columns is-multiline" style="text-align:center">
              <div class="column is-12">
                <h3 class="title is-5 mb-2 mr-1">Data Upload Lembar Kerja Sub Lingkup Thermohygrometer </h3>
              </div>
            </div>
...
useHead({
  title: 'Lembar Kerja Thermohygrometer - ' + import.meta.env.VITE_PROJECT,
})
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-thermohygrometer?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-thermohygrometer?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
      ...
      reader.onload = (e) => {
        ...
        const groups: {
          ...
          data: {
            ...
            set_poin: any;
            set_poin_satuan: any;
            penunjukan_standar: any;
            penunjukan_standar_satuan: any;
            pembacaan_alat: any;
            pembacaan_alat_satuan: any;
            koreksi: any;
            koreksi_satuan: any;
            ketidakpastian: any;
...
const Save = async () => {
  ...
  if (!item.value.resolusi) { H.alert('warning', 'Resolusi °C harus di isi'); return }
  if (!item.value.resolusipersen) { H.alert('warning', 'Resolusi %rH harus di isi'); return }
  if (!item.value.rentangukursuhu) { H.alert('warning', 'Rentang Ukur °C harus di isi'); return }
  if (!item.value.rentangukursuhupersen) { H.alert('warning', 'Rentang Ukur %rH harus di isi'); return }
  ...
  if (!item.value.kondisiPengukuranSuhu) { H.alert('warning', 'Kondisi Pengukuran Suhu harus di isi'); return }
  if (!item.value.kondisiPengukuranKelembaban) { H.alert('warning', 'Kondisi Pengukuran Kelembaban harus di isi'); return }
  ...
  let json = {
    ...
    'resolusi': item.value.resolusi,
    'resolusipersen': item.value.resolusipersen,
    'rentangukursuhu': item.value.rentangukursuhu,
    'rentangukursuhupersen': item.value.rentangukursuhupersen,
    'suhu': item.value.suhu,
    'kelembabanRelatif': item.value.kelembabanRelatif,
    'kondisipengukuransuhu': item.value.kondisiPengukuranSuhu,
    'kondisipengukurankelembaban': item.value.kondisiPengukuranKelembaban,
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/penyelia/save-data-upload-lembar-kerja-thermohygrometer`, json).then((response: any) => {
      isLoadingSave.value = false
...
const detailOrder = async () => {
  ...
  item.value.resolusipersen = data.resolusipersen
  item.value.rentangukursuhu = data.rentangukursuhu
  item.value.rentangukursuhupersen = data.rentangukursuhupersen
  item.value.suhu = data.suhulembarkerja
  item.value.kelembabanRelatif = data.kelembabanRelatiflembarkerja
  item.value.kondisiPengukuranSuhu = data.kondisipengukuransuhu
  item.value.kondisiPengukuranKelembaban = data.kondisipengukurankelembaban
