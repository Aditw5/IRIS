              ...
              <div class="column is-3 mt-4">
                <VCardCustom :style="'padding:5px 25px'">
                  <div class="label-status success">
                    ...
                    <span class="ml-1">TOTAL DATA CLAMP</span>
                  </div>
...
const downloadTemplate = () => {
  window.open('/service/penyelia/download-template-lembar-kerja-clamp?token=' + useUserSession().token, '_blank');
}
...
const fetchData = async () => {
  ...
  await useApi().get(`penyelia/get-lembar-kerja-clamp?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
...
const Save = async () => {
  ...
  isLoadingSave.value = true;
  await useApi().post(
    `/penyelia/save-data-upload-lembar-kerja-clamp`, json).then((response: any) => {
      isLoadingSave.value = false
