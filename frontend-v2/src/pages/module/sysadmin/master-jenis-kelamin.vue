<template>
  ...
  <div class="columns column">
    <h3 class="title is-5 mb-2 mr-1">Jenis Kelamin</h3>
  </div>
          ...
          <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
            <div class="tile-grid-item">
              <div class="tile-grid-item-inner">
                ...
                <div class="meta">
                  <span class="dark-inverted">{{ item.jeniskelamin }}</span>
            ...
            <div class="column is-12">
              <VField label="Jenis Kelamin">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.jeniskelamin" placeholder="Jenis Kelamin" class="is-rounded" />
                </VControl>
              </VField>
            </div>
...
useHead({
  title: 'Jenis Kelamin - ' + import.meta.env.VITE_PROJECT,
})
const item: any = ref({
  aktif: true
})
...
const dataSourcefiltered = computed(() => {
  ...
  return dataSource.value.filter((items: any) => {
    return (
      items.jeniskelamin.match(new RegExp(filters.value, 'i'))
    )
  })
})
...
async function fetchData() {
  ...
  const response = await useApi().get(
    '/sysadmin/master-jenis-kelamin?offset=' + offset +
    '&limit=' + limit +
    '&rows=' + rows +
    StatusEnabled + JenisKelamin 
  )
  isLoading = false
...
async function save() {
  ...
  var objSave =
  {
    'jeniskelamin': {
      ...
      'jeniskelamin': item.value.jeniskelamin,
      'statusenabled': item.value.statusenabled ? item.value.statusenabled : null,
    }
  }
  isLoadingTT.value = true
  await useApi().post(
    `/sysadmin/save-jenis-kelamin`, objSave).then((response: any) => {
...
async function deleterow(e: any) {
  await useApi().post(
    `/sysadmin/delete-jenis-kelamin`, { 'id': e.id }).then((response: any) => {
      fetchData ()
    }, (error) => {
    })
}
