<template>
  <VCard>
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1">Vendor</h3>
    </div>
      ...
      <div class="column is-8">
        ...
        <div class="user-grid user-grid-v2" v-if="selectView === 'list'">
          <DataTable :value="dataSourcefiltered" class="p-datatable-sm" :paginator="true" :rows="10"
            ...
            <Column field="namavendor" header="Nama Vendor" :sortable="true"></Column>
            <Column field="alamatvendor" header="Alamat Vendor" :sortable="true"></Column>
            <Column field="nohp" header="No HP Vendor" :sortable="true"></Column>
            ...
            <div v-for="(gItem, key) in dataSourcefiltered" :key="key" class="column is-6">
              <div class="tile-grid-item">
                <div class="tile-grid-item-inner">
                  ...
                  <div class="meta">
                    <span class="dark-inverted">{{ gItem.nohp }}</span>
                  </div>
                  <VTag :color="gItem.status_c" :label="gItem.status" style="margin-left:90px" />
                  <VDropdown icon="feather:more-vertical" spaced right>
                    <template #content>
                      <a role="menuitem" class="dropdown-item is-media" @click="detail(gItem)">
                        <div class="icon">
        ...
        <VCard>
          <div class="columns is-multiline">
            ...
            <div class="column is-12">
              <VField label="Nama Vendor">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.namavendor"
                    @update:modelValue="v => (item.namavendor = toUp(v))" placeholder="Nama Vendor"
            ...
            <div class="column is-12">
              <VField label="Alamat Vendor">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.alamatvendor" @update:modelValue="v => (item.alamatvendor = toUp(v))"
                    placeholder="Alamat Vendor" class="is-rounded uppercase" autocomplete="off" />
                </VControl>
              </VField>
            </div>

            <div class="column is-12">
              <VField label="No HP Vendor">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.nohp" @update:modelValue="v => (item.nohp = toUp(v))"
                    placeholder="No Hp Vendor" class="is-rounded uppercase" autocomplete="off" />
                </VControl>
              </VField>
            </div>
...
useHead({
  title: 'Master Vendor - ' + import.meta.env.VITE_PROJECT,
})
...
const dataSourcefiltered = computed(() => {
  ...
  return dataSource.value.filter((row: any) => {
    return [
      toUp(row.nohp),
      toUp(row.namavendor),
      toUp(row.alamatvendor),
    ].some((col) => col.includes(f))
  })
})
...
async function fetchData() {
  ...
  try {
    ...
    if (item.value.namavendor) {
      namavendor = '&namavendor=' + encodeURIComponent(toUp(item.value.namavendor))
    }

    StatusEnabled = '&statusenabled=' + (item.value.aktif ? 'true' : 'false')

    const response = await useApi().get(
      '/sysadmin/master-vendor?offset=' +
      offset +
      '&limit=' +
      limit +
      '&rows=' +
...
function edit(e: any) {
  item.value.id = e.id
  item.value.namavendor = toUp(e.namavendor)
  item.value.alamatvendor = toUp(e.alamatvendor)
  item.value.nohp = toUp(e.nohp)
...
async function save() {
  if (!item.value.namavendor) {
    useToaster().error('Nama Vendor harus di isi')
    return
  }
  if (!item.value.alamatvendor) {
    useToaster().error('Alamat Vendor harus di isi')
    return
  }
  const objSave = {
    datastandar: {
      id: item.value.id ?? '',
      namavendor: toUp(item.value.namavendor),
      alamatvendor: toUp(item.value.alamatvendor),
      nohp: toUp(item.value.nohp) ?? null,
  ...
  await useApi()
    .post(`/sysadmin/save-vendor`, objSave)
    .then(
      () => {
...
async function deleterow(e: any) {
  await useApi()
    .post(`/sysadmin/delete-vendor`, { id: e.id })
    .then(
      () => {
        fetchData()
      },
...
function clear() {
  item.value.id = ''
  item.value.namavendor = ''
  item.value.alamatvendor = ''
  item.value.nohp = ''
}
...
watch(
  ...
  (val) => {
    ...
    if (val.startsWith("08")) {
      item.value.nohp = "628" + val.slice(2)
    }
    else if (val.startsWith("0")) {
      item.value.nohp = "628" + val.slice(1)
    }
    else if (val.startsWith("8")) {
      item.value.nohp = "628" + val.slice(1)
    }
    item.value.nohp = item.value.nohp.replace(/[^\d]/g, "")
  }
)
