<template>
  ...
  <div class="columns column">
    <h3 class="title is-5 mb-2 mr-1">Serial Number</h3>
  </div>
    ...
    <div class="column is-8">
      ...
      <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
        <DataTable :value="dataSourcefiltered" class="p-datatable-sm" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
          ...
          <Column field="namaserialnumber" header="Nama Serial Number" :sortable="true"></Column>
          ...
          <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
            <div class="tile-grid-item">
              <div class="tile-grid-item-inner">
                ...
                <div class="meta">
                  <span class="dark-inverted">{{ item.namaserialnumber }}</span>
        ...
        <VCard>
          <div class="columns is-multiline">
            ...
            <div class="column is-12">
              <VField label="Nama Serial Number">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.namaserialnumber" placeholder="Nama Serial Number" class="is-rounded" />
                </VControl>
              </VField>
            </div>
...
useHead({
  title: 'Merk Alat - ' + import.meta.env.VITE_PROJECT,
})
const item: any = ref({
  aktif: true
})
...
const dataSourcefiltered = computed(() => {
  ...
  return dataSource.value.filter((items: any) => {
    return (
      items.namaserialnumber.match(new RegExp(filters.value, 'i'))
    )
  })
})
...
async function fetchData() {
  ...
  let namaserialnumber = ''
  let StatusEnabled = '' 

  if (item.namaserialnumber) namaserialnumber = '&namaserialnumber' + item.namaserialnumber
  item.value.aktif ? StatusEnabled = '&statusenabled=' + item.value.aktif : StatusEnabled = '&statusenabled=false'

  const response = await useApi().get(
    '/sysadmin/master-sn-alat?offset=' + offset +
    '&limit=' + limit +
    '&rows=' + rows +
    StatusEnabled + namaserialnumber
  )
  isLoading = false
...
async function save() {
  ...
  var objSave =
  {
    'namaserialnumber': {
      ...
      'namaserialnumber': item.value.namaserialnumber,
      'statusenabled': item.value.statusenabled ? item.value.statusenabled : null,
    }
  }
  isLoadingTT.value = true
  await useApi().post(
    `/sysadmin/save-sn-alat`, objSave).then((response: any) => {
...
async function deleterow(e: any) {
  await useApi().post(
    `/sysadmin/delete-sn-alat`, { 'id': e.id }).then((response: any) => {
      fetchData ()
    }, (error) => {
    })
}
