            ...
            <Column field="Aksi" style="min-width: 120px; text-align: center;">
              <template #body="slotProps">
                <VIconButton v-tooltip.top.left="'Setujui'" color="success" outlined circle icon="fas fa-check"
                  :disabled="Number(slotProps.data.statusorderpembayaran) !== 0"
                  @click="confirmApproval(slotProps.data, 17)" />
                <VIconButton v-tooltip.top.left="'Tolak'" class="ml-3" color="danger" outlined circle icon="feather:x"
                  :disabled="Number(slotProps.data.statusorderpembayaran) !== 0"
                  @click="confirmApproval(slotProps.data, 18)" />
              </template>
            </Column>
  ...
  <div :class="['modal', modalApproval ? 'is-active' : '']">
    ...
    <div class="modal-card" style="width: 740px; height: 600px; max-width: 92vw">
      ...
      <section class="modal-card-body">
        <template v-if="approvalStatusBaru === 17">
          <h4 class="title is-6 mb-3">PEMBAYARAN</h4>
          <div class="columns is-multiline">
            <div class="column is-4">
              <VField label="Status (Paid/No" class="is-rounded-select_Z is-autocomplete-select">
                <VControl icon="fa:search" fullwidth class="prime-auto ">
                  <AutoComplete v-model="f.statuspay" :suggestions="d_statuspaidpbj"
                    @complete="fetchStatusPaidPBJ($event)" :optionLabel="'label'" :dropdown="true" :minLength="3"
            ...
            <div class="column is-4">
              <VField class="vreq" label="No BKK (Bukti Kas Keluar)">
                <VControl>
            ...
            <div class="column is-4">
              <VField label="VERIFIKASI INVOICE">
                <VDatePicker v-model="f.verifikasiinvoice" is24hr mode="dateTime" style="width: 100%;">
          ...
          <VField label="Keterangan (opsional)">
            <VControl>
              <textarea class="textarea" rows="4" v-model="approvalNote"
                placeholder="Contoh: Disetujui, invoice dan BKK valid."></textarea>
            </VControl>
          </VField>
        </template>
        <template v-else>
...
useHead({ title: 'Kendali PBJ Pembayaran - ' + import.meta.env.VITE_PROJECT })
...
const fetchStatusPaidPBJ = async (filter: any) => {
  const res = await useApi().get(
    `general/dropdown/statuspaidpbj_m?select=id,statuspaid&param_search=statuspaid&query=${filter.query}&limit=10`
  )
  d_statuspaidpbj.value = res
}

const mapAsmanStatus = (val: any) => {
  const n = Number(val)
  if (n === 19) return { text: 'Disetujui', color: 'success' }
  if (n === 20) return { text: 'Ditolak', color: 'danger' }
...
const decorateRows = (rows: any[]) =>
  (rows || []).map((e: any, i: number) => {
    e.no = i + 1
    const m = mapAsmanStatus(e.statusorderpembayaran)
    e.statusText = m.text
    e.color = m.color
    return e
  })
...
const fetchDataPengajuan = async () => {
  ...
  try {
    const res = await useApi().get(`pbj/get-pengajuan-pbj-pembayaran?${search}`)
    res.forEach((e: any) => {
...
const fetchDataRiwayat = async () => {
  ...
  const search = item.value.search ? `&search=${encodeURIComponent(item.value.search)}` : ''
  try {
    const res = await useApi().get(`pbj/get-riwayat-pbj-pembayaran?${search}`)
    res.forEach((e: any) => {
      e.detailList = (e.detailList || []).map((d: any, idx: number) => ({
        ...d,
        no: idx + 1,
...
const approvalStatusBaru = ref<17 | 18 | 0>(0)
const approvalStatusBaruLabel = computed(() => (approvalStatusBaru.value === 17 ? 'Persetujuan' : 'Penolakan'))
const approvalNote = ref('')
const approvalError = ref('')
const approvalSubmitting = ref(false)

const f = ref<{
  verifikasiinvoice: boolean | null
  nobkk: string
  statuspay: any
}>({
  verifikasiinvoice: null,
  nobkk: '',
  statuspay: '',
})

const confirmApproval = (row: any, statusBaru: 17 | 18) => {
  const isApprove = statusBaru === 17
  confirm.require({
...
const openApprovalModal = (row: any, statusBaru: 17 | 18) => {
...
const submitApprovalWithNote = async () => {
  ...
  if (approvalStatusBaru.value === 17) {
    if (f.value.verifikasiinvoice === null) { H.alert('warning', 'Verifikasi Invoice harus dipilih'); return }
    if (!f.value.nobkk) { H.alert('warning', 'No BKK harus di isi'); return }
    if (!f.value.statuspay) { H.alert('warning', 'Status (Paid/No) harus dipilih'); return }
  } else if (approvalStatusBaru.value === 18) {
  ...
  const payload: any = {
    ...
    statusorderpembayaran: approvalStatusBaru.value,
    ketverifpembayaran: approvalNote.value || null,
    verifikasiinvoice: H.formatDate(f.value.verifikasiinvoice, 'YYYY-MM-DD hh:mm:ss'),
    nobkk: f.value.nobkk || null,
    statuspembayaran: f.value.statuspay.value || null,
  }
  approvalSubmitting.value = true
  try {
    await useApi().post('pbj/approval-pembayaran', payload)
    closeApprovalModal()
