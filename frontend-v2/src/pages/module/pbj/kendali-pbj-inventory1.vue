<template>
        ...
        <div v-else>
          <DataTable v-model:expandedRows="expandedRows" dataKey="norec" :value="dataSourcePengajuan"
            ...
            <Column field="Aksi" style="min-width: 120px; text-align: center;">
              <template #body="slotProps">
                <VIconButton v-tooltip.top.left="'Setujui'" color="success" outlined circle icon="fas fa-check"
                  :disabled="Number(slotProps.data.statusordinven1) !== 0"
                  @click="confirmApproval(slotProps.data, 7)" />
                <VIconButton v-tooltip.top.left="'Tolak'" class="ml-3" color="danger" outlined circle icon="feather:x"
                  :disabled="Number(slotProps.data.statusordinven1) !== 0"
                  @click="confirmApproval(slotProps.data, 8)" />
              </template>
            </Column>
            ...
            <Column field="tglpengajuan" style="min-width: 240px">
              <template #body="slotProps">
                <span v-if="slotProps.data.tglpengajuan">
                  {{ H.formatDateToLocalString(slotProps.data.tglpengajuan) }}
                </span>
                <span v-else>-</span>
              </template>
            </Column>
      ...
      <div class="column" v-else>
        ...
        <div v-else>
          <DataTable v-model:expandedRows="expandedRows" dataKey="norec" :value="dataSourceRiwayat"
            ...
            <Column field="tglpembuatanpr" style="min-width: 240px">
              <template #body="slotProps">
                <span v-if="slotProps.data.tglpembuatanpr">{{ H.formatDateToLocalString(slotProps.data.tglpembuatanpr) }}</span>
                <span v-else>-</span>
              </template>
            </Column>
            ...
            <Column field="tglpenerimaanro" style="min-width: 240px">
              <template #body="slotProps">
                <span v-if="slotProps.data.tglpenerimaanro">{{ H.formatDateToLocalString(slotProps.data.tglpenerimaanro) }}</span>
                <span v-else>-</span>
              </template>
            </Column>
            <Column field="tglpenerimaanhpe" style="min-width: 240px">
              <template #body="slotProps">
                <span v-if="slotProps.data.tglpenerimaanhpe">{{ H.formatDateToLocalString(slotProps.data.tglpenerimaanhpe) }}</span>
                <span v-else>-</span>
              </template>
            </Column>
            <Column field="tglpenerimaanrks" style="min-width: 240px">
              <template #body="slotProps">
                <span v-if="slotProps.data.tglpenerimaanrks">{{ H.formatDateToLocalString(slotProps.data.tglpenerimaanrks) }}</span>
                <span v-else>-</span>
              </template>
            </Column>
            <Column field="tglpenyerahandokumen" style="min-width: 240px">
              <template #body="slotProps">
                <span v-if="slotProps.data.tglpenyerahandokumen">{{ H.formatDateToLocalString(slotProps.data.tglpenyerahandokumen) }}</span>
                <span v-else>-</span>
              </template>
            </Column>
            ...
            <Column field="tglaanwijzing" style="min-width: 240px">
              <template #body="slotProps">
                <span v-if="slotProps.data.tglaanwijzing">{{ H.formatDateToLocalString(slotProps.data.tglaanwijzing) }}</span>
                <span v-else>-</span>
              </template>
            </Column>
            ...
            <Column field="tglpembukaanpenawaran" style="min-width: 240px">
              <template #body="slotProps">
                <span v-if="slotProps.data.tglpembukaanpenawaran">{{ H.formatDateToLocalString(slotProps.data.tglpembukaanpenawaran) }}</span>
                <span v-else>-</span>
              </template>
            </Column>
            ...
            <Column field="tglmulairealisasi" style="min-width: 240px">
              <template #body="slotProps">
                <span v-if="slotProps.data.tglmulairealisasi">{{ H.formatDateToLocalString(slotProps.data.tglmulairealisasi) }}</span>
                <span v-else>-</span>
              </template>
            </Column>
            <Column field="tglselesairealisasi" style="min-width: 240px">
              <template #body="slotProps">
                <span v-if="slotProps.data.tglselesairealisasi">{{ H.formatDateToLocalString(slotProps.data.tglselesairealisasi) }}</span>
                <span v-else>-</span>
              </template>
            </Column>
            ...
            <Column field="verifikasiinvoice" style="min-width: 240px">
              <template #body="slotProps">
                <span v-if="slotProps.data.verifikasiinvoice">{{ H.formatDateToLocalString(slotProps.data.verifikasiinvoice) }}</span>
                <span v-else>-</span>
              </template>
            </Column>
  ...
  <div :class="['modal', modalApproval ? 'is-active' : '']">
    ...
    <div class="modal-card" style="width: 740px; height: 740px; max-width: 92vw">
      ...
      <section class="modal-card-body">
        <template v-if="approvalStatusBaru === 7">
          <div class="approval-grid">

            <div class="field custom-field">
              <label class="label">Tanggal Terima Berkas PR (wajib)</label>
              <VDatePicker v-model="approvalTglTerimaBerkasPR" is24hr mode="dateTime">
                <template #default="{ inputValue, inputEvents }">
                  <VControl icon="feather:calendar">
                    <VInput :value="inputValue" v-on="inputEvents" placeholder="Pilih Tanggal" />
                  </VControl>
                </template>
              </VDatePicker>
            </div>
            ...
            <div class="field custom-field">
              <label class="label">Tanggal pembuatan dan approval RO</label>
              <VDatePicker v-model="approvalTglRO" is24hr mode="dateTime">
                <template #default="{ inputValue, inputEvents }">
                  <VControl icon="feather:calendar">
                    <VInput :value="inputValue" v-on="inputEvents" placeholder="Pilih Tanggal" />
                  </VControl>
            ...
            <div class="field custom-field">
              <label class="label">Tanggal pembuatan dan approval HPE</label>
              <VDatePicker v-model="approvalTglHPE" is24hr mode="dateTime">
                <template #default="{ inputValue, inputEvents }">
                  <VControl icon="feather:calendar">
                    <VInput :value="inputValue" v-on="inputEvents" placeholder="Pilih Tanggal" />
                  </VControl>
            ...
            <div class="field custom-field">
              <label class="label">Tanggal pembuatan dan approval RKS</label>
              <VDatePicker v-model="approvalTglRKS" is24hr mode="dateTime">
                <template #default="{ inputValue, inputEvents }">
                  <VControl icon="feather:calendar">
                    <VInput :value="inputValue" v-on="inputEvents" placeholder="Pilih Tanggal" />
                  </VControl>
            ...
            <div class="field custom-field" style="grid-column: span 2;">
              <label class="label">Tanggal Penyerahan Dokumen (RO, HPE, RKS)</label>
              <VDatePicker v-model="approvalTglPenyerahanDokumen" is24hr mode="dateTime">
                <template #default="{ inputValue, inputEvents }">
                  <VControl icon="feather:calendar">
                    <VInput :value="inputValue" v-on="inputEvents" placeholder="Pilih Tanggal" />
                  </VControl>
          ...
          <div class="field mt-5">
            ...
            <VControl>
              <textarea class="textarea" rows="3" v-model="approvalNote"
...
useHead({ title: 'Kendali PBJ Ineventory - ' + import.meta.env.VITE_PROJECT })
...
const mapAsmanStatus = (val: any) => {
  const n = Number(val)
  if (n === 7) return { text: 'Disetujui', color: 'success' }
  if (n === 8) return { text: 'Ditolak', color: 'danger' }
...
const decorateRows = (rows: any[]) =>
  (rows || []).map((e: any, i: number) => {
    e.no = i + 1
    const m = mapAsmanStatus(e.statusordinven1)
    e.statusText = m.text
    e.color = m.color
    return e
  })
...
const fetchDataPengajuan = async () => {
  ...
  try {
    const res = await useApi().get(`pbj/get-pengajuan-pbj-inven1?${search}`)
    res.forEach((e: any) => {
...
const fetchDataRiwayat = async () => {
  ...
  const search = item.value.search ? `&search=${encodeURIComponent(item.value.search)}` : ''
  try {
    const res = await useApi().get(`pbj/get-riwayat-pbj-inven1?${search}`)
    res.forEach((e: any) => {
      e.detailList = (e.detailList || []).map((d: any, idx: number) => ({
        ...d,
        no: idx + 1,
...
const approvalStatusBaru = ref<7 | 8 | 0>(0)
const approvalStatusBaruLabel = computed(() => (approvalStatusBaru.value === 7 ? 'Persetujuan' : 'Penolakan'))
const approvalNote = ref('')
const approvalError = ref('')
const approvalSubmitting = ref(false)
const approvalTglTerimaBerkasPR = ref<Date | null>(null)
const approvalTglRO = ref<any>(null)
const approvalTglHPE = ref<any>(null)
const approvalTglRKS = ref<any>(null)
const approvalTglPenyerahanDokumen = ref<any>(null)
...
const confirmApproval = (row: any, statusBaru: 7 | 8) => {
  const isApprove = statusBaru === 7
  confirm.require({
...
const openApprovalModal = (row: any, statusBaru: 7 | 8) => {
  approvalTargetRow.value = row
  approvalStatusBaru.value = statusBaru
  approvalNote.value = ''
  approvalError.value = ''
  approvalTglTerimaBerkasPR.value = null
  approvalTglRO.value = null
  approvalTglHPE.value = null
  approvalTglRKS.value = null
  approvalTglPenyerahanDokumen.value = null
...
const submitApprovalWithNote = async () => {
  ...
  if (approvalStatusBaru.value === 7 && isBlankDate(approvalTglTerimaBerkasPR.value)) {
    approvalError.value = 'Tanggal Penerimaan Berkas PR wajib diisi untuk persetujuan.'
    return
  }
  if (approvalStatusBaru.value === 7 && isBlankDate(approvalTglRO.value)) {
    approvalError.value = 'Tanggal Penerimaan RO wajib diisi untuk persetujuan.'
    return
  }
  if (approvalStatusBaru.value === 7 && isBlankDate(approvalTglHPE.value)) {
    approvalError.value = 'Tanggal Penerimaan HPE wajib diisi untuk persetujuan.'
    return
  }
  if (approvalStatusBaru.value === 7 && isBlankDate(approvalTglRKS.value)) {
    approvalError.value = 'Tanggal Penerimaan RKS wajib diisi untuk persetujuan.'
    return
  }
  if (approvalStatusBaru.value === 7 && isBlankDate(approvalTglPenyerahanDokumen.value)) {
    approvalError.value = 'Tanggal Penyerahan Dokumen wajib diisi untuk persetujuan.'
    return
  }

  if (!approvalTargetRow.value || approvalStatusBaru.value === 0) return

  const tglPenerimaanBerkasPRStr =
    approvalStatusBaru.value === 7 && approvalTglTerimaBerkasPR.value
      ? toSqlLocal(approvalTglTerimaBerkasPR.value)
      : null

  const tglPenerimaanROStr =
    approvalStatusBaru.value === 7 && approvalTglRO.value
      ? toSqlLocal(approvalTglRO.value)
      : null

  const tglPenerimaanHPEStr =
    approvalStatusBaru.value === 7 && approvalTglHPE.value
      ? toSqlLocal(approvalTglHPE.value)
      : null

  const tglPenerimaanRKSStr =
    approvalStatusBaru.value === 7 && approvalTglRKS.value
      ? toSqlLocal(approvalTglRKS.value)
      : null

  const tglPenyerahanDokumenStr =
    approvalStatusBaru.value === 7 && approvalTglPenyerahanDokumen.value
      ? toSqlLocal(approvalTglPenyerahanDokumen.value)
      : null

  approvalSubmitting.value = true
  try {
    await useApi().post('pbj/approval-inven1', {
      norec: approvalTargetRow.value.norec,
      statusordinven1: approvalStatusBaru.value,
      ketverifinven1: approvalNote.value || null,
      tglpenerimaanberkaspr: H.formatDate(tglPenerimaanBerkasPRStr, 'YYYY-MM-DD hh:mm:ss'),
      tglpenerimaanro: H.formatDate(tglPenerimaanROStr, 'YYYY-MM-DD hh:mm:ss'),
      tglpenerimaanhpe: H.formatDate(tglPenerimaanHPEStr, 'YYYY-MM-DD hh:mm:ss'),
      tglpenerimaanrks: H.formatDate(tglPenerimaanRKSStr, 'YYYY-MM-DD hh:mm:ss'),
      tglpenyerahandokumen: H.formatDate(tglPenyerahanDokumenStr, 'YYYY-MM-DD hh:mm:ss'),
    })
...
/* Pastikan grid punya jarak yang cukup tinggi agar label 2 baris tidak bertabrakan */
.approval-grid {
  display: grid;
  grid-template-columns: 1fr 1fr;
  gap: 25px 20px;
  align-items: end;
  /* Input akan sejajar di bawah meskipun tinggi label berbeda */
}

/* Memaksa label untuk tidak memotong teks dan menyesuaikan tinggi */
.custom-field .label {
...
/* Perbaikan untuk mobile agar grid jadi 1 kolom */
@media (max-width: 768px) {
  .approval-grid {
