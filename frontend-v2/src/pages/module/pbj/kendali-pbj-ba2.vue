            ...
            <Column field="Aksi" style="min-width: 120px; text-align: center;">
              <template #body="slotProps">
                <VIconButton v-tooltip.top.left="'Setujui'" color="success" outlined circle icon="fas fa-check"
                  :disabled="Number(slotProps.data.statusorderba2) !== 0"
                  @click="confirmApproval(slotProps.data, 15)" />
                <VIconButton v-tooltip.top.left="'Tolak'" class="ml-3" color="danger" outlined circle icon="feather:x"
                  :disabled="Number(slotProps.data.statusorderba2) !== 0"
                  @click="confirmApproval(slotProps.data, 16)" />
              </template>
            </Column>
  ...
  <div :class="['modal', modalApproval ? 'is-active' : '']">
    ...
    <div class="modal-card" style="width: 740px; height: 600px; max-width: 92vw">
      ...
      <section class="modal-card-body">
        <template v-if="approvalStatusBaru === 15">
          <h4 class="title is-6 mb-3">SEKRETARIS TIM BA</h4>
          <div class="columns is-multiline">
            <div class="column is-12">
              <VField class="vreq" label="No BA">
                <VControl>
            ...
            <div class="column is-6">
              <VField class="vreq" label="Posisi BA">
                <VControl>
            ...
            <div class="column is-6">
              <VField class="vreq" label="Status">
                <VControl>
...
useHead({ title: 'Kendali PBJ BA 2 - ' + import.meta.env.VITE_PROJECT })
...
const mapAsmanStatus = (val: any) => {
  const n = Number(val)
  if (n === 17) return { text: 'Disetujui', color: 'success' }
  if (n === 18) return { text: 'Ditolak', color: 'danger' }
...
const decorateRows = (rows: any[]) =>
  (rows || []).map((e: any, i: number) => {
    e.no = i + 1
    const m = mapAsmanStatus(e.statusorderba2)
    e.statusText = m.text
    e.color = m.color
    return e
  })
...
const fetchDataPengajuan = async () => {
  ...
  try {
    const res = await useApi().get(`pbj/get-pengajuan-pbj-ba2?${search}`)
    res.forEach((e: any) => {
...
const fetchDataRiwayat = async () => {
  ...
  const search = item.value.search ? `&search=${encodeURIComponent(item.value.search)}` : ''
  try {
    const res = await useApi().get(`pbj/get-riwayat-pbj-ba2?${search}`)
    res.forEach((e: any) => {
      e.detailList = (e.detailList || []).map((d: any, idx: number) => ({
        ...d,
        no: idx + 1,
...
const approvalStatusBaru = ref<15 | 16 | 0>(0)
const approvalStatusBaruLabel = computed(() => (approvalStatusBaru.value === 15 ? 'Persetujuan' : 'Penolakan'))
...
const confirmApproval = (row: any, statusBaru: 15 | 16) => {
  const isApprove = statusBaru === 15
  confirm.require({
...
const openApprovalModal = (row: any, statusBaru: 15 | 16) => {
...
const submitApprovalWithNote = async () => {
  ...
  if (approvalStatusBaru.value === 15) {
    if (!f.value.noba) { H.alert('warning', 'No BA harus di isi'); return }
    if (!f.value.posisiba) { H.alert('warning', 'Posisi BA harus di isi'); return }
    if (!f.value.statusba) { H.alert('warning', 'Status harus di isi'); return }
  } else if (approvalStatusBaru.value === 16) {
  ...
  const payload: any = {
    ...
    statusorderba2: approvalStatusBaru.value,
    ketverifba2: approvalNote.value || null,
    noba: f.value.noba || null,
    posisiba: f.value.posisiba || null,
    statusba: f.value.statusba || null,
  }
  approvalSubmitting.value = true
  try {
    await useApi().post('pbj/approval-ba2', payload)
    closeApprovalModal()
