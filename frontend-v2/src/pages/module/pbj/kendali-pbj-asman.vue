<template>
  ...
  <div class="column">
      ...
      <div class="column" v-else>
        ...
        <div v-else>
          <DataTable v-model:expandedRows="expandedRows" dataKey="norec" :value="dataSourcePengajuan"
            ...
            <Column field="Aksi" style="min-width: 120px; text-align: center;">
              <template #body="slotProps">
                <VIconButton v-tooltip.top.left="'Setujui'" color="success" outlined circle icon="fas fa-check"
                  :disabled="Number(slotProps.data.statusorderasman) !== 0"
                  @click="confirmApproval(slotProps.data, 1)" />
                <VIconButton v-tooltip.top.left="'Tolak'" class="ml-3" color="danger" outlined circle icon="feather:x"
                  :disabled="Number(slotProps.data.statusorderasman) !== 0"
                  @click="confirmApproval(slotProps.data, 2)" />
              </template>
            </Column>
            ...
            <Column field="namalengkap" style="min-width: 240px" />
            <Column field="judulpermintaan" style="min-width: 240px" />
            <Column field="pemohonpbj" style="min-width: 200px" />
            <Column field="pengadaanpbj" style="min-width: 200px" />
            <Column field="tglpengajuan" style="min-width: 240px">
              <template #body="slotProps">
                <span>{{ H.formatDateToLocalString(slotProps.data.tglpengajuan) }}</span>
              </template>
            </Column>
            ...
            <Column field="prk" style="min-width: 200px" />
            <Column field="wo" style="min-width: 200px" />
            <Column field="project" style="min-width: 200px" />
            <Column field="noproyek" style="min-width: 200px" />
            <Column field="costcode" style="min-width: 200px" />
    ...
    <VCard class="mt-5">
      <div class="is-flex is-align-items-center is-justify-content-space-between mb-2">
        <h3 class="title is-5 mb-2">Pengajuan BA 1</h3>
        <div class="is-flex is-align-items-center" style="gap:.5rem">
          <VTag :color="'warning'" rounded>Diajukan BA 1</VTag>
        </div>
      </div>
      ...
      <div class="column" v-else>
        <VPlaceholderPage v-if="dataSourcePengajuanBa1.length == 0" title="Belum ada pengajuan PBJ" larger>
          <template #image>
            <img class="light-image" src="/@src/assets/illustrations/placeholders/search-1.svg" alt="" />
            <img class="dark-image" src="/@src/assets/illustrations/placeholders/search-1-dark.svg" alt="" />
          </template>
        ...
        <div v-else>
          <DataTable v-model:expandedRows="expandedRows" dataKey="norec" :value="dataSourcePengajuanBa1"
            class="p-datatable-sm" :loading="isLoading" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
            scrollable
            paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown"
            responsiveLayout="stack" breakpoint="960px" sortMode="multiple"
            ...
            <template #header>
              <div class="flex flex-wrap align-items-center justify-content-between gap-2">
                <div class="is-flex is-align-items-center" style="gap:.5rem">
                  ...
                  <VButton color="info" class="mb-3" icon="feather:refresh-ccw" outlined
                    @click="fetchDataPengajuanBa1()">
                    Refresh
                  </VButton>
                </div>
              </div>
            ...
            <Column field="Aksi" style="min-width: 120px; text-align: center;">
              <template #body="slotProps">
                <VIconButton v-tooltip.top.left="'Setujui'" color="success" outlined circle icon="fas fa-check"
                  :disabled="Number(slotProps.data.statusorderasmanba1) !== 0"
                  @click="confirmApprovalBa1(slotProps.data, 13)" />
                <VIconButton v-tooltip.top.left="'Tolak'" class="ml-3" color="danger" outlined circle icon="feather:x"
                  :disabled="Number(slotProps.data.statusorderasmanba1) !== 0"
                  @click="confirmApprovalBa1(slotProps.data, 14)" />
              </template>
            </Column>
            <Column style="min-width: 100px">
              <template #body="slotProps">
              ...
              <template #body="{ data: row }">
                <div class="progress-cell">
                  ...
                  <div class="progress-stepper">
                    ...
                    <div class="step-caption">Step {{ Number(row.progress_step || 1) }}/12</div>
                  </div>
                </div>
              </template>
            </Column>
  ...
  <div :class="['modal', modalApproval ? 'is-active' : '']">
    ...
    <div class="modal-card" style="width: 640px; max-width: 92vw">
      ...
      <section class="modal-card-body">
        <VField :label="'Keterangan' + (approvalStatusBaru === 2 ? ' (wajib)' : ' (opsional)')">
          <VControl>
            <textarea class="textarea" rows="5" v-model="approvalNote"
              :placeholder="approvalStatusBaru === 1 ? 'Contoh: Disetujui oleh atasan, silakan lanjut proses.' : 'Contoh: Ditolak karena alasan ... (wajib diisi)'"></textarea>
          </VControl>
        </VField>
      </section>
  ...
  <div :class="['modal', modalApprovalBa1 ? 'is-active' : '']">
    <div class="modal-background" @click="closeApprovalModalBa1()"></div>
    <div class="modal-card" style="width: 720px; max-width: 95vw">
      <header class="modal-card-head">
        <p class="modal-card-title">{{ approvalStatusBaruBa1Label }}</p>
        <button class="delete" aria-label="close" @click="closeApprovalModalBa1()"></button>
      </header>

      <section class="modal-card-body">
        <div v-if="approvalStatusBaruBa1 === 13">
          <div class="columns">
            <div class="column is-6">
              <VField label="Nilai SLA (wajib)">
                <VControl>
                  <input class="input" type="number" min="0" step="1" v-model.number="ba1NilaiSla"
          ...
          <VField label="Konfirmasi/Validasi Rendal Proyek (wajib)">
            <VControl>
              <textarea class="textarea" rows="3" v-model="ba1KonfirmasiRendal"
                placeholder="Contoh: Selesai/OK/divalidasi oleh Rendal Proyek"></textarea>
            </VControl>
          </VField>

          <VField label="Status dari Katimpro (wajib)">
            <VControl>
              <input class="input" type="text" v-model="ba1StatusKatimpro" placeholder="Contoh: Selesai" />
            </VControl>
          </VField>
        </div>

        <div v-else>
          <VField label="Keterangan Penolakan (wajib)">
            <VControl>
              <textarea class="textarea" rows="4" v-model="ba1RejectNote"
                placeholder="Jelaskan alasan penolakan BA 1"></textarea>
            </VControl>
          </VField>
        </div>
      </section>
      ...
      <footer class="modal-card-foot is-justify-content-space-between">
        <div>
          <VButton color="dark" outlined icon="feather:x" @click="closeApprovalModalBa1()">Batal</VButton>
        </div>
        <div>
          <VButton :loading="approvalBa1Submitting" color="primary" raised icon="feather:check"
            @click="submitApprovalBa1()">
            Simpan
          </VButton>
        </div>
      </footer>
...
useHead({ title: 'Kendali PBJ - ' + import.meta.env.VITE_PROJECT })
...
const dataSourcePengajuanBa1: any = ref([])
...
const mapAsmanStatus = (val: any) => {
  const n = Number(val)
  if (n === 1) return { text: 'Disetujui', color: 'success' }
  if (n === 2) return { text: 'Ditolak', color: 'danger' }
  return { text: 'Diajukan', color: 'warning' }
}

const mapAsmanStatusBa1 = (val: any) => {
  const n = Number(val)
  if (n === 15) return { text: 'Disetujui', color: 'success' }
  if (n === 16) return { text: 'Ditolak', color: 'danger' }
  return { text: 'Diajukan', color: 'warning' }
}
...
const decorateRows = (rows: any[]) =>
  (rows || []).map((e: any, i: number) => {
    e.no = i + 1
    const m = mapAsmanStatus(e.statusorderasman)
    e.statusText = m.text
    e.color = m.color
    return e
  })

const decorateRowsBa1 = (rows: any[]) =>
  (rows || []).map((e: any, i: number) => {
    e.no = i + 1
    const m = mapAsmanStatusBa1(e.statusorderasmanba1)
    e.statusText = m.text
    e.color = m.color
    return e
  })
...
const fetchDataPengajuan = async () => {
  ...
  try {
    const res = await useApi().get(`pbj/get-pengajuan-pbj-asman?${search}`)
    res.forEach((e: any) => {
...
const fetchDataPengajuanBa1 = async () => {
  isPlaceLoad.value = true
  const search = item.value.search ? `&search=${encodeURIComponent(item.value.search)}` : ''
  try {
    const res = await useApi().get(`pbj/get-pengajuan-pbj-asmanba1?${search}`)
    res.forEach((e: any) => {
      e.detailList = (e.detailList || []).map((d: any, idx: number) => ({ ...d, no: idx + 1 }))
    })
    dataSourcePengajuanBa1.value = decorateRowsBa1(res)
  } catch (e) {
    dataSourcePengajuanBa1.value = []
  } finally {
    isPlaceLoad.value = false
  }
}
...
const fetchDataRiwayat = async () => {
  ...
  const search = item.value.search ? `&search=${encodeURIComponent(item.value.search)}` : ''
  try {
    const res = await useApi().get(`pbj/get-riwayat-pbj-asman?${search}`)
    res.forEach((e: any) => {
      e.detailList = (e.detailList || []).map((d: any, idx: number) => ({ ...d, no: idx + 1 }))
    })
...
const approvalStatusBaru = ref<1 | 2 | 0>(0)
const approvalStatusBaruLabel = computed(() => (approvalStatusBaru.value === 1 ? 'Persetujuan' : 'Penolakan'))
...
const openApprovalModal = (row: any, statusBaru: 1 | 2) => {
...
const submitApprovalWithNote = async () => {
  if (approvalStatusBaru.value === 2 && !approvalNote.value.trim()) {
    H.alert('warning', 'Keterangan penolakan wajib diisi')
    return
  }
  ...
  try {
    await useApi().post('pbj/approval-asman', {
      norec: approvalTargetRow.value.norec,
      statusorderasman: approvalStatusBaru.value,
      ketverifasman: approvalNote.value || null,
    })
    closeApprovalModal()
    await Promise.all([fetchDataPengajuan(), fetchDataRiwayat()])
  } catch (e) {
    H.alert('warning', 'Gagal menyimpan approval. Coba lagi.')
  } finally {
    approvalSubmitting.value = false
  }
}

const modalApprovalBa1 = ref(false)
const approvalBa1TargetRow: any = ref(null)
const approvalStatusBaruBa1 = ref<13 | 14 | 0>(0)
const approvalStatusBaruBa1Label = computed(() => (approvalStatusBaruBa1.value === 13 ? 'Persetujuan BA 1' : 'Penolakan BA 1'))
const ba1NilaiSla = ref<number | null>(null)
const ba1KonfirmasiRendal = ref('')
const ba1StatusKatimpro = ref('')
const ba1RejectNote = ref('')
const approvalBa1Submitting = ref(false)

const confirmApprovalBa1 = (row: any, statusBaru: 13 | 14) => {
  const isApprove = statusBaru === 13
  confirm.require({
    message: `Yakin ingin ${isApprove ? 'MENYETUJUI' : 'MENOLAK'} pengajuan BA 1 ini?`,
    header: isApprove ? 'Konfirmasi Persetujuan BA 1' : 'Konfirmasi Penolakan BA 1',
    icon: 'pi pi-question-circle',
    acceptLabel: isApprove ? 'Setujui' : 'Tolak',
    rejectLabel: 'Batal',
    acceptClass: isApprove ? 'p-button-success' : 'p-button-danger',
    rejectClass: 'p-button-text',
    accept: () => openApprovalModalBa1(row, statusBaru),
  })
}

const openApprovalModalBa1 = (row: any, statusBaru: 13 | 14) => {
  approvalBa1TargetRow.value = row
  approvalStatusBaruBa1.value = statusBaru
  ba1NilaiSla.value = null
  ba1KonfirmasiRendal.value = ''
  ba1StatusKatimpro.value = ''
  ba1RejectNote.value = ''
  modalApprovalBa1.value = true
}

const closeApprovalModalBa1 = () => {
  modalApprovalBa1.value = false
  approvalBa1TargetRow.value = null
  approvalStatusBaruBa1.value = 0
  ba1NilaiSla.value = null
  ba1KonfirmasiRendal.value = ''
  ba1StatusKatimpro.value = ''
  ba1RejectNote.value = ''
...
const submitApprovalBa1 = async () => {
  if (!approvalBa1TargetRow.value || approvalStatusBaruBa1.value === 0) return

  if (approvalStatusBaruBa1.value === 13) {
    if (ba1NilaiSla.value === null || isNaN(Number(ba1NilaiSla.value))) {
      H.alert('warning', 'Nilai SLA wajib diisi')
      return
    }
    if (!ba1KonfirmasiRendal.value.trim()) {
      H.alert('warning', 'Konfirmasi/Validasi Rendal Proyek wajib diisi')
      return
    }
    if (!ba1StatusKatimpro.value.trim()) {
      H.alert('warning', 'Status dari Katimpro wajib diisi')
      return
    }
  } else if (approvalStatusBaruBa1.value === 14) {
    if (!ba1RejectNote.value.trim()) {
      H.alert('warning', 'Keterangan penolakan wajib diisi')
      return
    }
  }

  approvalBa1Submitting.value = true
  try {
    await useApi().post('pbj/approval-asmanba1', {
      norec: approvalBa1TargetRow.value.norec,
      statusorderasmanba1: approvalStatusBaruBa1.value,
      ketverifasmanba1: approvalStatusBaruBa1.value === 13 ? ba1RejectNote.value : null,
      nilai_sla: approvalStatusBaruBa1.value === 13 ? ba1NilaiSla.value : null,
      konfirmasi_rendal: approvalStatusBaruBa1.value === 13 ? ba1KonfirmasiRendal.value : null,
      status_katimpro: approvalStatusBaruBa1.value === 13 ? ba1StatusKatimpro.value : null,
    })
    closeApprovalModalBa1()
    await Promise.all([fetchDataPengajuanBa1(), fetchDataRiwayat()])
  } catch (e) {
    H.alert('warning', 'Gagal menyimpan approval BA 1. Coba lagi.')
  } finally {
    approvalBa1Submitting.value = false
  }
}
