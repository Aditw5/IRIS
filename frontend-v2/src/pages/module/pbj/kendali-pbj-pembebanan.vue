            ...
            <Column field="Aksi" style="min-width: 120px; text-align: center;">
              <template #body="slotProps">
                <VIconButton v-tooltip.top.left="'Setujui'" color="success" outlined circle icon="fas fa-check"
                  :disabled="Number(slotProps.data.statusorderpembebanan) !== 0"
                  @click="confirmApproval(slotProps.data, 19)" />
                <VIconButton v-tooltip.top.left="'Tolak'" class="ml-3" color="danger" outlined circle icon="feather:x"
                  :disabled="Number(slotProps.data.statusorderpembebanan) !== 0"
                  @click="confirmApproval(slotProps.data, 20)" />
              </template>
            </Column>
  ...
  <div :class="['modal', modalApproval ? 'is-active' : '']">
    ...
    <div class="modal-card" style="width: 740px; height: 600px; max-width: 92vw">
      ...
      <section class="modal-card-body">
        <template v-if="approvalStatusBaru === 19">
          <h4 class="title is-6 mb-3">PEMBEBANAN</h4>
          <div class="columns is-multiline">
            <div class="column is-6">
              <VField class="vreq" label="No Jurnal">
                <VControl>
            ...
            <div class="column is-6">
              <VField class="vreq" label="Periode">
                <VControl>
          ...
          <VField label="Keterangan (opsional)">
            <VControl>
              <textarea class="textarea" rows="4" v-model="approvalNote"
                placeholder="Contoh: Disetujui, pembebanan dicatat."></textarea>
            </VControl>
          </VField>
        </template>
        <template v-else>
...
useHead({ title: 'Kendali PBJ Pembebanan - ' + import.meta.env.VITE_PROJECT })
...
const mapPembayaranStatus = (val: any) => {
  const n = Number(val)
  if (n === 21) return { text: 'Disetujui', color: 'success' }
  if (n === 22) return { text: 'Ditolak', color: 'danger' }
...
const decorateRows = (rows: any[]) =>
  (rows || []).map((e: any, i: number) => {
    e.no = i + 1
    const m = mapPembayaranStatus(e.statusorderpembebanan)
    e.statusText = m.text
    e.color = m.color
    return e
  })
...
const fetchDataPengajuan = async () => {
  ...
  try {
    const res = await useApi().get(`pbj/get-pengajuan-pbj-pembebanan?${search}`)
    res.forEach((e: any) => {
...
const fetchDataRiwayat = async () => {
  ...
  const search = item.value.search ? `&search=${encodeURIComponent(item.value.search)}` : ''
  try {
    const res = await useApi().get(`pbj/get-riwayat-pbj-pembebanan?${search}`)
    res.forEach((e: any) => {
      e.detailList = (e.detailList || []).map((d: any, idx: number) => ({ ...d, no: idx + 1 }))
    })
...
const approvalStatusBaru = ref<19 | 20 | 0>(0)
const approvalStatusBaruLabel = computed(() => (approvalStatusBaru.value === 19 ? 'Persetujuan' : 'Penolakan'))
...
const confirmApproval = (row: any, statusBaru: 19 | 20) => {
  const isApprove = statusBaru === 19
  confirm.require({
...
const openApprovalModal = (row: any, statusBaru: 19 | 20) => {
  approvalTargetRow.value = row
  approvalStatusBaru.value = statusBaru
  approvalNote.value = ''
  approvalError.value = ''
  f.value = { nojurnal: '', periode: '' }
...
const submitApprovalWithNote = async () => {
  ...
  if (approvalStatusBaru.value === 19) {
    if (!f.value.nojurnal) { H.alert('warning', 'No Jurnal harus di isi'); return }
    if (!f.value.periode) { H.alert('warning', 'Periode harus di isi'); return }
  } else if (approvalStatusBaru.value === 20) {
  ...
  const payload: any = {
    ...
    statusorderpembebanan: approvalStatusBaru.value,
    ketverifpembebanan: approvalNote.value || null,
    nojurnal: f.value.nojurnal || null,
    periode: f.value.periode || null,
  }
  approvalSubmitting.value = true
  try {
    await useApi().post('pbj/approval-pembebanan', payload)
    closeApprovalModal()
