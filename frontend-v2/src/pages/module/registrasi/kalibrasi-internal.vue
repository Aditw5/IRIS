<template>
      ...
      <div class="form-body">
        <div class="form-section is-grey">
          ...
          <div class="form-section-inner is-horizontal">
            ...
            <VField horizontal label="Alat Milik Lab" class="is-rounded-select_Z  is-autocomplete-select">
          ...
          <Fieldset legend="Daftar Alat" :toggleable="true">
            <div class="mt-5 form-section-inner is-horizontal">
              ...
              <div class="table-responsive">
                <table class="table-po" width="100%">
                  <thead>
                    <tr class="tr-po">
                      <th class="th-po" width="25%" style="text-align:center;">Nama Alat
                        <VField>
                          <VControl>
                            <VIconButton class="ml-2" icon="feather:folder-plus" rounded @click="openInstruksiKerja()"
                              v-tooltip.bottom.right="'Master Alat'" />
                          </VControl>
                        </VField>
                      </th>
                          ...
                          <VField>
                            <VControl>
                              <div style="display:flex; align-items:center">
                                <AutoComplete v-model="items.alat" :suggestions="d_alatulab" @complete="fetchAlatUlab"
                                  optionLabel="label" :dropdown="true" :minLength="3" class="is-input" appendTo="body"
                                  loadingIcon="pi pi-spinner" field="label" placeholder="ketik untuk mencari..." />
  ...
  <Dialog v-model:visible="isInstruksi" modal header="Master Alat" :style="{ width: '80vw' }">
    <MasterAlat :hide="true" />
  </Dialog>
</template>
...
import MasterAlat from './../sysadmin/master-produk.vue';
...
const d_alatulab = ref<{ label: string; value: any }[]>([])
const d_lokasikalibrasi = ref<{ label: string; value: any }[]>([])
const d_ulab = ref<{ label: string; value: any }[]>([])
const d_lingkup = ref<{ label: string; value: any }[]>([])
const d_penyelia = ref<{ label: string; value: any }[]>([])
const d_pelaksana = ref<{ label: string; value: any }[]>([])
...
async function onScanSuccess(decodedText: string) {
  ...
  const { data } = await useApi().get(
    `registrasi/produk-by-id?idmitra=${item.ulab.value}&id_alat=${id_alat}`
  )
  if (!data.length) {
    scannerItem.value = null
    return
  ...
  d_alatulab.value = [option, ...d_alatulab.value.filter(o => o.value !== option.value)]
...
const fetchAlatUlab = async (filter: any) => {
  if (!item.ulab) { H.alert('warning', 'Pemilik Standar harus di isi'); return }
  await useApi().get(
    `registrasi/produk-by-id?idmitra=${item.ulab.value}&param_search=namaproduk,namamerk,namatipe,namaserialnumber&query=${filter.query}`
  ).then((response) => {
    d_alatulab.value = response.data.map((e: any) => {
      return {
        label: `${e.namaproduk} - ${e.namamerk} ${e.namatipe} (${e.namaserialnumber})`,
        value: e.id
      };
...
const saveRegistrasi = async () => {
  ...
  await useApi().post(`/registrasi/save-registrasi-kalibrasi-ulab`, formData).then(async (response: any) => {
