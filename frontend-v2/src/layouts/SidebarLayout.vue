...
let data = useStorage('core_listMenu', [])
...
core_listMenu.value = data.value
</script>
...
<template>
  <div class="sidebar-layout">
    ...
    <MobileNavbar :is-open="isMobileSidebarOpen" @toggle="isMobileSidebarOpen = !isMobileSidebarOpen">
      <template #brand>
        <RouterLink :to="{ name: 'index' }" class="navbar-item is-brand">
          <AnimatedLogoULAB width="38px" height="38px" />
        </RouterLink>
    ...
    <Sidebar :theme="props.theme" :is-open="isDesktopSidebarOpen">
      <template #links>
        <li>
          <a :class="[activeMobileSubsidebar === 'menu' && 'is-active']" data-content="menu" aria-label="View Nav"
            tabindex="0" @keydown.space.prevent="switchSidebar('menu')" @click="switchSidebar('menu')">
