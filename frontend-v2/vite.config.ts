...
const SITEMAP_HOST = process.env.SITEMAP_HOST || 'https://ulabumro.id'
...
const publicDirectoryIndexPlugin = () => ({
  ...
  configureServer(server) {
    server.middlewares.use((req, _res, next) => {
      ...
      const target = publicIndexes[pathname]
...
export default defineConfig({
  ...
  // Jangan hapus asset lama saat build agar user yang sedang membuka web tidak stuck setelah deploy
  build: {
    emptyOutDir: false,
    minify: true,
  ...
  plugins: [
    ...
    Pages({
      ...
      onRoutesGenerated: (routes) =>
        generateSitemap({
          routes,
          hostname: SITEMAP_HOST.endsWith('/')
            ? SITEMAP_HOST
            : SITEMAP_HOST + '/',
        }),
    }),
    ...
    /**
     * PWA / Service Worker dimatikan.
     * Tujuannya agar browser user tidak menyimpan cache frontend lama
     * yang menyebabkan harus refresh berkali-kali setelah deploy.
     */
    // VitePWA({
    //   base: '/',
     ...
     *
     * @see https://github.com/Jonasruckenberg/vite-imagetools
     */
    imagetools({
      silent: SILENT,
    }),
